...
var TPARSE = {
  ...
  getHomePageOwnerHandle: function() {
    return ES6.SPECIAL.getTwitterHomePageOwnerHandle();
  },
  ...
  getThrottledRetryElem: function() {
    const col = TPARSE.getMainColumn();
    if (!col) { return undefined; }
    const cells = col.querySelectorAll('div[data-testid="cellInnerDiv"]');
    if (!cells || cells.length === 0) { return undefined; }
    const lastCell = cells[cells.length-1];
    if (lastCell.innerText && lastCell.innerText.startsWith('Something went wrong.')) {
