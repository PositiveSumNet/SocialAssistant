...
var MASTODON = {
  ...
  onReceivedLoggedInUser: function(data) {
    ...
    const userAvatarElm = RENDER.PERSON.renderPerson(_mdonConnectedUser, RENDER_CONTEXT.PERSON.AUTHD_USER, RENDER_CONTEXT.ANCHORS.ALL, false);
  ...
  getLoggedInUser: function(onSuccess) {
    ...
    .then(function(response) { 
      ...
      response.json().then(function(data) {
        ...
        .catch(function(err) {
          // Failed to fetch
          // fyi: if we ever need to, we could start using an image proxy like
          // https://corsproxy.io/?https://mastodon.scot/avatars/original/missing.png
          console.log('Failed to fetch image');
        })
        .finally(function() {
          // the auth itself worked (whether or not the image fetch failed), so this is put in the finally clause
  ...
  saveConnections: function(ownerAccountId, ppl, followDirection) {
    ...
    // _worker will receive this message, do the save, and call us back on-save to increment the saved counter
    _worker.postMessage( { 
      records: ppl, 
      pageType: pageType, 
  ...
  onSearchedRemoteAccountOwners: function(owners) {
    ...
    if (owners.length === 1 && !_deletingMdonRemoteOwner) {
      ...
      document.getElementById('txtMdonDownloadConnsFor').value = STR.stripPrefix(owners[0].Handle, '@');
      MASTODON.onChooseRemoteOwner();
    }
    else {
      for (i = 0; i < owners.length; i++) {
        // renderPerson uses DOMPurify.sanitize
        mdonRemoteOwnerPivotPicker.innerHTML += RENDER.PERSON.renderPerson(owners[i], 'owner', RENDER_CONTEXT.PERSON.ACCOUNT_OWNER);
      }
  ...
  onChooseRemoteOwner: function() {
    MASTODON.searchRemoteAccounts(document.getElementById('txtMdonDownloadConnsFor').value, MASTODON.onResolvedChosenRemoteOwner, 1, 1);
  ...
  renderNetwork: function(forceRefresh) {
    ...
    QUERYWORK_UI.executeSearch(forceRefresh);
  },
