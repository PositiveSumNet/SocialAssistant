...
var TWEET_SAVE_MAPPER = {
  ...
  mapRecordIntoSet: function(record, set, graph) {
    ...
    let srchUrlKey = urlKey;
    let srchAuthorHandle = record[SAVABLE_TWEET_ATTR.authorHandle];
    let srchAuthorName = record[SAVABLE_TWEET_ATTR.authorName];
    let srchPostText = record[SAVABLE_TWEET_ATTR.postText];
    let srchQtAuthorHandle;
    let srchQtAuthorName;
    let srchQtPostText;
    ...
    if (record[SAVABLE_TWEET_ATTR.quoteTweet] && record[SAVABLE_TWEET_ATTR.quoteTweet][SAVABLE_TWEET_ATTR.urlKey]) {
      ...
      srchQtAuthorHandle = quoteTweet[SAVABLE_TWEET_ATTR.authorHandle];
      srchQtAuthorName = quoteTweet[SAVABLE_TWEET_ATTR.authorName];
      srchQtPostText = quoteTweet[SAVABLE_TWEET_ATTR.postText];
    }
    ...
    const searchBlob = TWEET_SAVE_MAPPER.buildSearchBlob(srchUrlKey, srchAuthorHandle, srchAuthorName, srchPostText, srchQtAuthorHandle, srchQtAuthorName, srchQtPostText);
    
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostSearchBlob.Name).sogs.push({s: urlKey, o: searchBlob, g: graph});
  },

  buildSearchBlob: function(urlKey, authorHandle, authorName, postText, qtAuthorHandle, qtAuthorName, qtPostText) {
    const searchParts = [];
    searchParts.push(urlKey);
    searchParts.push(authorHandle);
    searchParts.push(authorName);
    searchParts.push(postText);
    searchParts.push(qtAuthorHandle);
    searchParts.push(qtAuthorName);
    searchParts.push(qtPostText);
...
var TCARD_SAVE_MAPPER = {
  ...
  mapRecordIntoSet: function(record, set, graph, parent) {
    const urlKey = record[TWEET_CARD_ATTR.urlKey];
    ...
    const srchUrlKey = urlKey;
    const srchCardText = record[TWEET_CARD_ATTR.cardText];
    const srchCardFullUrl = record[TWEET_CARD_ATTR.fullSourceUrl];
    ...
    const searchBlob = TCARD_SAVE_MAPPER.buildCardSearchBlob(srchUrlKey, srchCardText, srchCardFullUrl);
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostCardSearchBlob.Name).sogs.push({s: urlKey, o: searchBlob, g: graph});
  },

  buildCardSearchBlob: function(urlKey, cardText, fullUrl) {
    const searchParts = [];
    searchParts.push(urlKey);
    searchParts.push(cardText);
    searchParts.push(fullUrl);
...
var TPOST_MEDIA_SAVE_MAPPER = {
  ...
  mapSavableSet: function(records, pageType, graph) {
    ...
    for (let i = 0; i < records.length; i++) {
      let record = records[i];
      let urlKey = record[TWEET_POST_IMG_ATTR.urlKey];
