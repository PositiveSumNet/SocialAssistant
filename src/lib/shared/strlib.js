var STR = {
  ...
  padLeft: function(str, chr, totalLen) {
    if (!STR.hasLen(str)) {
      str = '';
    }
    str = str.toString();
    chr = chr.toString();
    const gap = totalLen - str.length;
    if (gap <= 0) { return str; }
    for (let i = 0; i < gap; i++) {
      str = `${chr}${str}`;
    }
  ...
  dateFromMmDdYyyy: function(str, endOfDay) {
    ...
    const dt = new Date(`${yyyy}-${mm}-${dd}${time}`);
    return dt;
  },
  ...
  toBase64: function(str, utf8Adjust) {
    if (utf8Adjust == true) {
      var textEncoder = new TextEncoder();
      const data = textEncoder.encode(str);
      const base64 = CRYPTO.buf2Base64(data);
      return base64;
    }
    else {
      return btoa(str);
  ...
  fromBase64: function(b64, utf8Adjust) {
    if (utf8Adjust == true) {
  ...
  // e.g. "scafaria_status_1626289311682379779.mp4"
  isValidTwitterVideoFileName: function(fileName) {
    if (!fileName.endsWith('.mp4')) {
      return false;
    }

    var nameOnly = STR.stripSuffix(fileName, '.mp4');
    var parts = nameOnly.split('_status_');
    return (parts.length == 2);
  },

  // when a file is downloaded more than once, it shows up with " (1)" or 2 etc. 
  // e.g. "scafaria_status_1626566689864163329 (1).mp4"
  cleanDownloadedFileName: function(fileName) {
    if (fileName.indexOf(' (') < 0) {
      ...
      if (parts.length != 2) {
        // unexpected, but just leave it be
        console.log('surprised by download filename ' + fileName);
        return fileName;
      }
      else {
        const baseName = parts[0];
        const extParts = parts[1].split(').');
        const extPart = extParts[extParts.length - 1];
        const cleanName = `${baseName}.${extPart}`;
        return cleanName;
      }
    }
  },
  ...
  buildSquidlrUrl: function(urlKey, resolution, autoDownloadSansPopup) {
    ...
    const autoSuffix = autoDownloadSansPopup ? '_auto' : '';
    return `https://${SQUIDLR_URL}/download?url=${xUrl}#${resolution}${autoSuffix}`;
  },
  ...
  buildVideoFileNameFromUrlKey: function(urlKey) {
    urlKey = STR.stripSuffix(urlKey, QUOTED_SUFFIX);
