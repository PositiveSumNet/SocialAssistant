var SETTINGS = {
  ...
  RECORDING: {
    ...
    getThreadExpansionUrlKeys: async function(limit) {
      const kvps = await SETTINGS.getCacheKvps(STORAGE_PREFIX.THREAD_EXPANSION_URLKEY, limit);
      return kvps.map(function(kvp) { return kvp.val; });
    },

    saveThreadExpansionUrlKey: async function(threadUrlKey) {
      if (threadUrlKey) {
        const key = `${STORAGE_PREFIX.THREAD_EXPANSION_URLKEY}${threadUrlKey}`;
        chrome.storage.local.set({ [key]: threadUrlKey });
      }
    },
