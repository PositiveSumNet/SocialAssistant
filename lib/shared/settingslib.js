var SETTINGS = {
  ...
  SYNCFLOW: {
    SHOULD_RUN: 'shouldRunSync',
    LAST_STEP_TYPE: 'lastSyncStepType',
    MARKER: 'syncMarker',
    COMPLETED_OK: 'syncCompletedOk',
    STUCK_LAST_ERROR_MSG: 'syncStuckLastErrorMsg',
    STUCK_LAST_ERROR_WHEN: 'syncStuckLastErrorWhen',
    
    // this is marker text that indicates we're at the very end for that step type
    // e.g. if we page alphabetically, getting to here indicates we finished the last MARKER.
    // note: tidle is the highest "normal" ascii character
    END_MARKER: '~~~',
    ...
    getShouldRun: function() {
      return STR.isTruthy(localStorage.getItem(SETTINGS.SYNCFLOW.SHOULD_RUN));
    },

    setShouldRun: function(should) {
      localStorage.setItem(SETTINGS.SYNCFLOW.SHOULD_RUN, should);
    },

    getLastStepType: function() {
      return localStorage.getItem(SETTINGS.SYNCFLOW.LAST_STEP_TYPE);
    },

    setLastStepType: function(stepType) {
      localStorage.setItem(SETTINGS.SYNCFLOW.LAST_STEP_TYPE, stepType);
    },

    getMarker: function() {
      return localStorage.getItem(SETTINGS.SYNCFLOW.MARKER);
    },

    setMarker: function(marker) {
      localStorage.setItem(SETTINGS.SYNCFLOW.MARKER, marker);
    },

    getCompletedOk: function() {
      const dt = localStorage.getItem(SETTINGS.SYNCFLOW.COMPLETED_OK);
      if (!parseInt(dt)) { return null; }
      return new Date(parseInt(dt));
    },

    setCompletedOk: function() {
      localStorage.setItem(SETTINGS.SYNCFLOW.COMPLETED_OK, Date.now());
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_WHEN);
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_MSG);
    },

    getStuckLastErrorMsg: function() {
      return localStorage.getItem(SETTINGS.SYNCFLOW.STUCK_LAST_ERROR_MSG);
    },

    getStuckLastErrorWhen: function() {
      const dt = localStorage.getItem(SETTINGS.SYNCFLOW.STUCK_LAST_ERROR_WHEN);
      if (!parseInt(dt)) { return null; }
      return new Date(parseInt(dt));
    },

    getSecondsToPauseAfterRecentError: function(secondsDeemedRecent) {
      const when = SETTINGS.SYNCFLOW.getStuckLastErrorWhen();
    ...
    setStuckLastError: function(msg) {
      localStorage.setItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_WHEN, Date.now());
      localStorage.setItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_MSG, msg);
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.COMPLETED_OK);
    },

    resetLastOkLastErr: function() {
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_WHEN);
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.STUCK_LAST_ERROR_MSG);
      localStorage.removeItem(SETTINGS.STUCK_LAST_ERROR_MSG.COMPLETED_OK);
    }
