var SETTINGS = {
  ...
  GITHUB: {
    ...
    TOKEN_STATUS: 'githubTokenStatus',
    ...
    getSyncRepoName: async function() {
      const repo = await SETTINGS.getStorageValue(SETTINGS.GITHUB.SYNC_REPO);
      if (!STR.hasLen(repo)) {
  ...
  RECORDING: {
    ...
    VIDEO_EXTRACTION: {
      getEmbeddedVideoUrlKeys: async function(take, skip) {
        return await SETTINGS.RECORDING.getPrefixedCacheValues(STORAGE_PREFIX.EMBEDDED_VIDEO_URLKEY, take, skip);
      },
    ...
    THREAD_EXPANSION: {
      getExpandThreadUrlKeys: async function(take, skip) {
        return await SETTINGS.RECORDING.getPrefixedCacheValues(STORAGE_PREFIX.THREAD_EXPANSION_URLKEY, take, skip);
      },
    ...
    getPrefixedCacheValues: async function(prefix, take, skip) {
