var SETTINGS = {
  ...
  NITTER: {
    ...
    getNitterDomain: async function() {
      ...
      let speedTestSetting = await chrome.storage.local.get([SETTINGS.NITTER.SPEED_TEST.CACHE_KEY]);
      ...
      speedTestSetting = speedTestSetting[SETTINGS.NITTER.SPEED_TEST.CACHE_KEY];
      if (!speedTestSetting || speedTestSetting.length == 0) { 
        // surprising, since we run speed test
        console.log('Defaulting nitter');
        return defaultDomain; 
      ...
      const speedTest = JSON.parse(speedTestSetting);
