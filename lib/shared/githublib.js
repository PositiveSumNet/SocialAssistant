...
var GITHUB = {
  ...
  getFileJson: async function(remoteDir, remoteFileName, repoConnInfo, onErrorFn) {
    ...
    if (response && STR.isTruthy(response.ok)) {
      ...
      if (!data || !data.content) {
        onErrorFn({ msg: `Cannot read ${remoteDir}/${remoteFileName}` });
        return;
      }
  ...
  TREES: {
    ...
    getFileOrNext: async function(directory, repoConnInfo, repoType, fileName) {
      let files = await GITHUB.TREES.getFiles(directory, repoConnInfo, repoType);
      if (!files || files.length == 0) { return null; }
      const found = files.find(function(f) { return f.path == fileName; });
      if (found) { return found; }
      files = ES6.sortBy(files, 'path');
      // deliberately not using STR.hasLen here (to allow for the difference between first call for the directory vs. marker of '')
      if (!fileName) {
        return files[0];
      }
      else
  ...
  VIDEOS: {
    ...
    resolveB64DataUri: async function(postUrlKey) {
      ...
      const response = await GITHUB.tryGetFileResponse(fileUrl, repoConnInfo.headers);
      if (!response || !STR.isTruthy(response.ok)) { return null; }
      ...
      if (STR.hasLen(fileInfo.content)) {
        ...
        if (!blobResponse || !STR.isTruthy(blobResponse.ok)) {
          return null;
        }
        else {
  ...
  testGithubConnection: async function(onSuccess, onFailure, repoType) {
    ...
    if (!userResponse || !STR.isTruthy(userResponse.ok)) {
    ...
    if (!putFileResponse || !STR.isTruthy(putFileResponse.ok)) {
    ...
    if (!deleteFileResponse || !STR.isTruthy(deleteFileResponse.ok)) {
    ...
    let isPublic;
    if (!repoResponse || !STR.isTruthy(repoResponse.ok)) {
