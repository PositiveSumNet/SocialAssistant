...
var GITHUB = {
  ...
  SHAS: {
    ...
    getMetadataTree: async function(path, repoConnInfo) {
      ...
      const url = `https://api.github.com/repos/${repoConnInfo.userName}/${repoConnInfo.repoName}/git/trees/main:${encodedPath}`;
    ...
    BACKUP: {
      upsertPushable: async function(pushable, onSuccess, onFailure) {
        ...
        if (pushable[SYNCFLOW.PUSHABLE.dontPush] != true) {
          ...
          if (pushResult.success != true) {
            ...
            GITHUB.SYNC.clearInMemoryCache();
      ...
      pushWorker: async function(pushable, relPath, repoConnInfo) {
        ...
        if (!putFileResponse.ok) {
          ...
          return { success: false, reason: GITHUB.SYNC.ERROR_CODE.pushBackupFileFailed, pushable: pushable, rateLimit: rateLimit };
        }
        else {
          return { success: true };
        }
