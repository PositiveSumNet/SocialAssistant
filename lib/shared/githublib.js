...
var GITHUB = {
  ...
  SHAS: {
    clearInMemoryCache: function() {
      _ghTreePath = null;
      _ghTreeMeta = null;
    },

    getBlobSha: async function(relPath, repoConnInfo) {
      const fileName = STR.getFileNameFromFullName(relPath);
      if (!STR.hasLen(fileName)) { return null; }
      const pathOnly = STR.getPathFromFullName(relPath) || '/';
      if (!_ghTreeMeta || pathOnly != _ghTreePath) {
        _ghTreeMeta = await GITHUB.SHAS.getMetadataTree(pathOnly, repoConnInfo);
        _ghTreePath = pathOnly;
      }
      if (!_ghTreeMeta) { return null; }
      const files = _ghTreeMeta.tree;
    ...
    getMetadataTree: async function(path, repoConnInfo) {
      ...
      const response = await GITHUB.SYNC.tryGetFileResponse(url, repoConnInfo.headers);
      if (!response) { return null; }
      return await response.json();
    },

    // stackoverflow.com/questions/7225313/how-does-git-compute-file-hashes?rq=3
    // TODO: Make this robust to deal with emojis. Open S/O question:
    // https://stackoverflow.com/questions/77290492/calculate-git-file-blob-sha-handle-emojis-etc
    calcTextContentSha: async function(text) {
      // because our encoding call to toBase64 passes adjustEmojis:true
      const adjText = STR.encodeTextEmojis(text);
  ...
  SYNC: {
    ...
    getRepoConnInfo: async function(onFailure) {
    ...
    BACKUP: {
      upsertPushable: async function(pushable, onSuccess, onFailure) {
        ...
        const repoConnInfo = await GITHUB.SYNC.getRepoConnInfo(onFailure);
        if (!repoConnInfo) { return; }

        // check accessibility
        const rateLimit = await GITHUB.getRateLimit(repoConnInfo.token);
        if (GITHUB.SYNC.checkIfRateLimited(rateLimit, onFailure) == true) {
          return;
        }

        if (pushable[SYNCFLOW.PUSHABLE.dontPush] != true) {
          
          const relPath = pushable[SYNCFLOW.PUSHABLE.filePath];
          let pushResult = await GITHUB.SYNC.BACKUP.pushWorker(pushable, relPath, repoConnInfo);
          if (pushResult.success != true) {
            // clear blob sha cache and try once more
            console.log('clearing sha cache and retrying...');
            GITHUB.SHAS.clearInMemoryCache();
            // sleep a couple seconds before retrying
            await ES6.sleep(2000);
            pushResult = await GITHUB.SYNC.BACKUP.pushWorker(pushable, relPath, repoConnInfo);
            if (pushResult.success != true) {
              if (onFailure) {
      ...
      pushWorker: async function(pushable, relPath, repoConnInfo) {
        ...
        const encodedContent = STR.hasLen(plainTextContent) ? STR.toBase64(plainTextContent, true) : null;
        ...
        if (!putFileResponse.ok) {
          ...
          return { success: false, reason: GITHUB.SYNC.ERROR_CODE.pushBackupFileFailed, pushable: pushable };
        }
        else {
          return { success: true };
        }
  ...
  getRateLimit: async function(token) {
    const headers = GITHUB.HEADERS.buildJsonHeaders(token);
    const request = { method: "GET", headers: headers };
    const response = await GITHUB.SYNC.tryGetFileResponse("https://api.github.com/rate_limit", request.headers);
