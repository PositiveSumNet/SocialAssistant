...
var GITHUB = {
  ...
  TREES: {
    ...
    getFiles: async function(directory, repoConnInfo, repoType) {
      ...
      files = files.filter(function(f) { return f.type != 'dir'});
      return files;
    }
  },
  ...
  SHAS: {
    getBlobSha: async function(relPath, repoConnInfo, repoType) {
      const file = GITHUB.TREES.getFile(relPath, repoConnInfo, repoType);
      if (!file) { return null; }
      return file.sha;
    },
  ...
  SYNC: {
    ...
    BACKUP: {
      upsertPushable: async function(pushable, onSuccess, onFailure) {
        const repoType = pushable[SYNCFLOW.SYNCABLE.repoType] || GITHUB.REPO_TYPE.DATA;
        ...
        if (pushable[SYNCFLOW.SYNCABLE.dontSync] != true) {
          
          const relPath = pushable[SYNCFLOW.SYNCABLE.filePath];
        ...
        await onSuccess({ rateLimit: rateLimit, pushable: pushable }, SYNCFLOW.DIRECTION.BACKUP);
      },
      ...
      pushWorker: async function(pushable, relPath, repoConnInfo) {
        ...
        const plainTextContent = pushable[SYNCFLOW.SYNCABLE.content];
