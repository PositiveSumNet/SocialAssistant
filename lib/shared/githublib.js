...
var GITHUB = {
  ...
  SYNC: {
    ...
    BACKUP: {
      upsertPushable: async function(pushable, config, onSuccess, onFailure) {
        ...
        if (pushable[SYNCFLOW.PUSHABLE.dontPush] != true) {
          ...
          if (!putFileResponse.ok) {
            ...
            if (onFailure) {
              onFailure({ reason: GITHUB.SYNC.ERROR_CODE.pushBackupFileFailed, pushable: pushable, rateLimit: rateLimit });
              return;
            }
          }
        }
        ...
        await onSuccess({ rateLimit: rateLimit, pushable: pushable, config: config });
      }
    }
  },
