...
var _ghDataTreePath;
var _ghDataTreeMeta;
var _ghVideosTreePath;
var _ghVideosTreeMeta;
...
var GITHUB = {
  ...
  SHAS: {
    ...
    getBlobSha: async function(relPath, repoConnInfo, repoType) {
      ...
      let cachedTreeMeta = GITHUB.SHAS.getCachedTreeMeta(repoType);
      const cachedTreePath = GITHUB.SHAS.getCachedTreePath(repoType);
      if (!cachedTreeMeta || pathOnly != cachedTreePath) {
        const treeMeta = await GITHUB.SHAS.getMetadataTree(pathOnly, repoConnInfo);
        GITHUB.SHAS.cacheTree(treeMeta, pathOnly, repoType);
      }
      cachedTreeMeta = GITHUB.SHAS.getCachedTreeMeta(repoType);
      if (!cachedTreeMeta) { return null; }
      const files = cachedTreeMeta.tree;
  ...
  SYNC: {
    ...
    BACKUP: {
      upsertPushable: async function(pushable, onSuccess, onFailure) {
        const repoType = pushable[SYNCFLOW.PUSHABLE.repoType] || GITHUB.REPO_TYPE.DATA;
