...
var SYNCFLOW = {
  ...
  UI: {
    logStatus: function(direction, status) {
      switch (direction) {
        ...
        default:
          SYNCFLOW.renderSyncRestoreStatus(status);
          break;
      }
    }
  },
  ...
  renderSyncRestoreStatus: function() {
  ...
  resumeSync: function(direction) {
    ...
    GITHUB.SHAS.clearInMemoryCache(GITHUB.REPO_TYPE.DATA);
  ...
  onFetchedForBackup: async function(pushable) {
    ...
    await GITHUB.SYNC.BACKUP.upsertPushable(pushable, SYNCFLOW.onGithubPushedOk, GHCONFIG_UI.onGithubFailure);
  },
