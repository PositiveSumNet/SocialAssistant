...
var SYNCFLOW = {
  ...
  runSyncFlowWorker: function(direction) {
    ...
    SYNCFLOW.runStepWorker(direction, step);
  ...
  runStepWorker: function(direction, step) {
    switch (direction) {
      case SYNCFLOW.DIRECTION.BACKUP:
        SYNCFLOW.runBackupWorker(step);
        break;
      case SYNCFLOW.DIRECTION.RESTORE:
      default:
        SYNCFLOW.runRestoreWorker(direction, step);
        break;
    }
  },

  runBackupWorker: function(step) {
    ...
    switch (stepType) {
      case SYNCFLOW.STEP_TYPE.profileFavorites:
        pushable = SYNCFLOW.PUSHABLE_BUILDER.buildProfileFavorites(step);
        break;
      case SYNCFLOW.STEP_TYPE.profiles:
      case SYNCFLOW.STEP_TYPE.profileImg64s:
      case SYNCFLOW.STEP_TYPE.networkFollowings:
  ...
  runRestoreWorker: function(step) {
