...
var SYNCFLOW = {
  ...
  onGithubPushedOk: function(result) {
    const statusMsg = `${result.pushable[SYNCFLOW.PUSHABLE.filepath]} pushed!`;
    SYNCFLOW.UI.logStatus(SYNCFLOW.DIRECTION.BACKUP, statusMsg);
    SYNCFLOW.UI.renderRateLimit(result.rateLimit);
  },

  // called back by worker with content needed for backup
  onFetchedForBackup: async function(step, pushable) {
