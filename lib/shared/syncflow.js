...
var SYNCFLOW = {
  ...
  onGithubSyncStepOk: function(result, direction) {
    ...
    const step = pushable[SYNCFLOW.SYNCABLE.step];
    const config = step[SYNCFLOW.STEP.config];

    GHCONFIG_UI.renderRateLimit(rateLimit);

    SYNCFLOW.recordExecutionStep(direction, step, pushable[SYNCFLOW.SYNCABLE.endMarker]);
  ...
  PUSH_EXEC: {
    ...
    onFetchedForBackup: async function(pushable) {
      ...
      await GITHUB.SYNC.BACKUP.upsertPushable(pushable, SYNCFLOW.PUSH_EXEC.onGithubSyncStepOk, GHCONFIG_UI.onGithubFailure);
    },
  ...
  PULL_EXEC: {
    kickoffRestoreStep: async function(step) {
      ...
      if (!nextRemoteFile) {
        ...
        SYNCFLOW.PULL_EXEC.onGithubPulledOk(result);
        return;
      }
    ...
    buildNoMoreRemoteFilesResult: function(step, rateLimit) {
      const pullable = {};
      pullable[SYNCFLOW.SYNCABLE.step] = step;
      pullable[SYNCFLOW.SYNCABLE.endMarker] = END_MARKING;
  ...
  FILE_NAMER: {
    ...
    getPostImgsFileName: function(step, relevantMarker) {
      ...
      const delim = SYNCFLOW.FILE_NAMER.DELIM;
      relevantMarker = relevantMarker.replaceAll('/', '_');
      return `${network}${delim}${relevantMarker}-${SYNCFLOW.FILE_NAMER.PATH_PART.images}.${SYNCFLOW.FILE_NAMER.EXT.json}`.toLowerCase();
    },
  ...
  PUSHABLE_BUILDER: {
    ...
    buildPushableWorker: function(step, fnDbQuery, fnWriter, fnFileNamer, fnEndMarkerCalcer, markerCol, contentCheckCol) {
      ...
      pushable[SYNCFLOW.SYNCABLE.step] = step;
      pushable[SYNCFLOW.SYNCABLE.content] = content;
      pushable[SYNCFLOW.SYNCABLE.filePath] = path;
      pushable[SYNCFLOW.SYNCABLE.startMarker] = step.marker;
      pushable[SYNCFLOW.SYNCABLE.endMarker] = endMarker;

      if (!STR.hasLen(path)) {
        pushable[SYNCFLOW.SYNCABLE.dontSync] = true;
      }
    ...
    buildNetworkConns: function(step) {
      const pushable = SYNCFLOW.PUSHABLE_BUILDER.buildPushableWorker(
        ...
        SYNCFLOW.FILE_NAMER.getNetworkConnsFilePath,
