...
sync
  ...
  networks
    x
      x-followedby-scafaria.json
      x-followersof-scafaria.json
  posts (thread)
    x
...
var SYNCFLOW = {
  ...
  FILE_NAMER: {
    ...
    getProfileFavoritesFilePath: function(network) {
  ...
  PUSHABLE_BUILDER: {
    buildProfileFavorites: function(step) {
      ...
      const site = SYNCFLOW.getSite(network);
      const rows = CONNFETCHER.SYNC.getFavorites(site);
      const content = SYNCFLOW.PUSH_WRITER.writeProfileFavorites(rows);
      const path = SYNCFLOW.FILE_NAMER.getProfileFavoritesFilePath(network);
      var pushable = {};
