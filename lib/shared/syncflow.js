...
sync
  ...
  networks
    x
      following
        x-scafaria-following.txt
      followers
        x-scafaria-followers.txt
  posts (thread)
    x
...
var SYNCFLOW = {
  ...
  FILE_NAMER: {
    ...
    getNetworkFilePath: function(step, endMarker) {
      ...
      const delim = SYNCFLOW.FILE_NAMER.DELIM;
      const fileName = `${network}${delim}${endMarker}${delim}${followPathPart}.${SYNCFLOW.FILE_NAMER.EXT.txt}`;
  ...
  PUSH_WRITER: {
    ...
    writeProfileFavorites: function(rows) {
      return SYNCFLOW.PUSH_WRITER.asText(rows, SYNC_COL.FAVORITES.Handle);
    },

    writeNetwork: function(rows) {
      return SYNCFLOW.PUSH_WRITER.asText(rows, SYNC_COL.NETWORK.Connection, true);
    }
  },
  ...
  PUSHABLE_BUILDER: {
    buildPushableWorker: function(step, fnDbQuery, fnWriter, fnFileNamer, fnEndMarkerCalcer, markerCol) {
    ...
    buildProfileFavorites: function(step) {
      return SYNCFLOW.PUSHABLE_BUILDER.buildPushableWorker(
        step, 
