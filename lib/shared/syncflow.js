...
var SYNCFLOW = {
  ...
  getNetworks: function(config) {
    ...
    if (STR.isTruthy(config[ns.DO_MASTODON])) {
      types.push(SYNCFLOW.NETWORK.mdon);
    }
  ...
  resumeSync: function(direction) {
    ...
    SETTINGS.SYNCFLOW.setShouldRun(direction, true);
    SETTINGS.SYNCFLOW.resetLastOkLastErr(direction);
    const config = SETTINGS.SYNCFLOW.getConfig(direction);
    SYNCFLOW.runSyncFlowWorker(direction, config);
  },

  // resumes where left off
  runSyncFlowWorker: function(direction, config) {
    const shouldRun = SETTINGS.SYNCFLOW.getShouldRun(direction);
    if (!shouldRun) {
    ...
    SYNCFLOW.kickoffSyncStep(direction, step, config);
  },

  kickoffSyncStep: function(direction, step, config) {
    switch (direction) {
      case SYNCFLOW.DIRECTION.BACKUP:
  ...
  calcCompletedFullRun: function(step, config) {
    const syncStepTypes = SYNCFLOW.getStepTypes(config);
    const syncNetworks = SYNCFLOW.getNetworks(config);
  ...
  onGithubPushedOk: function(result) {
    ...
    const completedFullRun = SYNCFLOW.calcCompletedFullRun(step, config);
  ...
  onFetchedForBackup: async function(pushable, config) {
    // now actually push it!
    await GITHUB.SYNC.BACKUP.upsertPushable(pushable, config, SYNCFLOW.onGithubPushedOk, SYNCFLOW.UI.onGithubFailure);
  },
