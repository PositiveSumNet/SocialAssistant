...
var SYNCFLOW = {
  ...
  buildStatus: function(direction) {
    ...
    if (lastRunOk) {
      ...
      if (lastNoop == true) {
        logMsg = `${logMsg} (prior step skipped; content unchanged)`;
      }
  ...
  PULL_EXEC: {
    ...
    continueRestore: async function(request, rows) {
      ...
      if (dbSha && dbSha == request.remoteSha) {
        ...
        const repoType = GITHUB.REPO_TYPE.DATA;
        const repoConnInfo = await GITHUB.SYNC.getRepoConnInfo(GHCONFIG_UI.onGithubFailure, repoType);
        const json = await GITHUB.getFileJson(request.remoteDir, request.remoteFileName, repoConnInfo, GHCONFIG_UI.onGithubFailure);
        console.log(json);
        console.log('todo: pull for ' + request.remoteFileName);
      }
    },
