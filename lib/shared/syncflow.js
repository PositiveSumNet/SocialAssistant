...
var SYNCFLOW = {
  ...
  PUSH_EXEC: {
    onGithubPushedOk: function(result) {
      ...
      SYNCFLOW.recordExecutionStep(direction, step, pushable[SYNCFLOW.PUSHABLE.endMarker]);
  ...
  PULL_EXEC: {
    ...
    buildNoMoreRemoteFilesResult: function(step, rateLimit) {
      const pullable = {};
      pullable[SYNCFLOW.PULLABLE.step] = step;
      pullable[SYNCFLOW.PULLABLE.endMarker] = END_MARKING;
    ...
    onGithubPulledOk: function(result) {
      GHCONFIG_UI.setGithubConnFailureMsg('');
  
      const rateLimit = result.rateLimit;
      const pullable = result.pullable;
      const step = pullable[SYNCFLOW.PULLABLE.step];
      const config = step[SYNCFLOW.STEP.config];
      const direction = SYNCFLOW.DIRECTION.RESTORE;
  
      GHCONFIG_UI.renderRateLimit(rateLimit);
  
      SYNCFLOW.recordExecutionStep(direction, step, pullable[SYNCFLOW.PULLABLE.endMarker]);
