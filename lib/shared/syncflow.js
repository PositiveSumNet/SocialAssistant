...
var SYNCFLOW = {
  ...
  onGithubSyncStepOk: function(result, direction) {
    ...
    SYNCFLOW.recordExecutionStep(direction, step, syncable[SYNCFLOW.SYNCABLE.endMarker]);
  ...
  PUSH_EXEC: {
    ...
    buildPushable: function(step) {
      ...
      const content = SYNCFLOW.PUSH_WRITER.asJson(rows);
  ...
  PULL_EXEC: {
    kickoffRestoreStep: async function(step) {
      ...
      const lastFileName = STR.hasLen(step.startMarker) ? SYNCFLOW.FILE_NAMER.getFileName(step, step.startMarker) : null;
      const nextRemoteFile = await GITHUB.TREES.getNextFile(remoteDir, repoConnInfo, repoType, lastFileName);
      ...
      _worker.postMessage({
        ...
        pullStep: step,
        pushStep: pushStep,
        remotePath: nextRemoteFile.path,
        remoteSha: nextRemoteFile.sha,
        rateLimit: rateLimit
      });
    },

    // called back from worker
    continueRestore: async function(request, rows) {
      ...
      const remoteSha = request.remoteSha;
      const pullStep = request.pullStep;
      const rateLimit = request.rateLimit;
      if (dbSha && dbSha == remoteSha) {
        console.log('skipping identical pull of ' + request.remotePath);
        const nextPullableResult = SYNCFLOW.PULL_EXEC.buildNextPullableResult(pullStep, rows, rateLimit);
        SYNCFLOW.onGithubSyncStepOk(nextPullableResult, SYNCFLOW.DIRECTION.RESTORE);
      }
      else {
        // worker calls back to main thread and we'll continue pulling the file from github
        console.log('todo: pull for ' + request.remotePath);
      }
    },

    buildNextPullableResult: function(pullStep, rows, rateLimit) {
      const syncable = {};
    ...
    buildNoMoreRemoteFilesResult: function(step, rateLimit) {
      const syncable = {};
      syncable[SYNCFLOW.SYNCABLE.step] = step;
      syncable[SYNCFLOW.SYNCABLE.endMarker] = END_MARKING;
  ...
  FILE_NAMER: {
    ...
    getFileName: function(step, relevantMarker) {
      ...
      switch (stepType) {
        ...
        case SYNCFLOW.STEP_TYPE.networkFollowings:
        case SYNCFLOW.STEP_TYPE.networkFollowers:
          return SYNCFLOW.FILE_NAMER.getNetworkConnsFileName(step);
        case SYNCFLOW.STEP_TYPE.postTopicRatings:
    ...
    getFilePath: function(step, relevantMarker) {
      ...
      switch (stepType) {
        ...
        case SYNCFLOW.STEP_TYPE.networkFollowings:
        case SYNCFLOW.STEP_TYPE.networkFollowers:
          return SYNCFLOW.FILE_NAMER.getNetworkConnsFilePath(step);
        case SYNCFLOW.STEP_TYPE.postTopicRatings:
        ...
        case SYNCFLOW.STEP_TYPE.postImgs:
          return SYNCFLOW.FILE_NAMER.getPostImgsPath(step, relevantMarker);
        default:
          console.log('Unhandled restore type');
          return null;
      }
    ...
    getProfilesFileName: function(step) {
      const network = step.network;
      const delim = SYNCFLOW.FILE_NAMER.DELIM;
      return `${network}${delim}${SYNCFLOW.FILE_NAMER.PATH_PART.profiles}${delim}${step.marker}.${SYNCFLOW.FILE_NAMER.EXT.json}`.toLowerCase();
    },
    ...
    getProfileImgsFileName: function(step) {
      const network = step.network;
      const delim = SYNCFLOW.FILE_NAMER.DELIM;
      return `${network}${delim}${SYNCFLOW.FILE_NAMER.PATH_PART.avatars}${delim}${step.marker}.${SYNCFLOW.FILE_NAMER.EXT.json}`.toLowerCase();
    },
    ...
    getNetworkConnsFileName: function(step) {
      const network = step.network;
      const delim = SYNCFLOW.FILE_NAMER.DELIM;
      return `${network}${delim}${endMarker}${delim}${followPathPart}.${SYNCFLOW.FILE_NAMER.EXT.json}`.toLowerCase();
    },
    ...
    getNetworkConnsFilePath: function(step, relevantMarker) {
      ...
      const fileName = SYNCFLOW.FILE_NAMER.getNetworkConnsFileName(step);
