...
var SYNCFLOW = {
  ...
  buildStatus: function(direction) {
    ...
    if (lastRunOk) {
      ...
      let logMsg = `${properDirection} for ${lastNetwork} ${lastType}... ${marker}`;
      if (lastNoop == true) {
        logMsg = `${logMsg} (no sync required)`;
      }
  ...
  onGithubSyncStepOk: function(result, direction) {
    ...
    const noop = result[SYNCFLOW.SYNCABLE.dontSync];
  ...
  PULL_EXEC: {
    kickoffRestoreStep: async function(step) {
      ...
      const fileName = step.marker ? SYNCFLOW.FILE_NAMER.getFileName(step, step.marker) : null;
      const nextRemoteFile = await GITHUB.TREES.getFileOrNext(remoteDir, repoConnInfo, repoType, fileName);
    ...
    continueRestore: async function(request, rows) {
      ...
      if (dbSha && dbSha == remoteSha) {
        ...
        const pulledResult = SYNCFLOW.PULL_EXEC.buildPulledResult(pullStep, rows, rateLimit);
        pulledResult[SYNCFLOW.SYNCABLE.dontSync] = true;
