// background fetch (via offscreen approach)
// also se comments for the "flow" at offscreen.js
var BGFETCH_REQUEST = {
  
  logToBackground: function(data) {
    chrome.runtime.sendMessage(
      {
        actionType: MSGTYPE.TOBACKGROUND.LOG_ME,
        data: JSON.stringify(data)
      });
  },

  getBackgroundScrapeUrl: async function() {
    const setting = await chrome.storage.local.get(SETTINGS.BG_SCRAPE.SCRAPE_URL);
    if (!setting) { return null; }
    return setting[SETTINGS.BG_SCRAPE.SCRAPE_URL];
  },

  onCompleteBgScrape: function() {
    const parsedUrl = URLPARSE.parseUrl(document.location.href);
    // ensure the next one is kicked off
    chrome.runtime.sendMessage(
      {
        actionType: MSGTYPE.TOBACKGROUND.LETS_SCRAPE,
        lastScrapedParsedUrl: parsedUrl
      });
  },

  clearRequests: async function() {
    const all = await chrome.storage.local.get();
    const entries = Object.entries(all);

    for (const [key, val] of entries) { 
      if (key.startsWith(STORAGE_PREFIX.BG_SCRAPE)) {
        chrome.storage.local.remove(key);
      }
    }
  },

  cacheBgRequest: function(records, pageType) {
    const key = `${STORAGE_PREFIX.BG_SCRAPE}${Date.now().toString()}`;
    
    const toStore = {};
    toStore[BG_SCRAPE_TYPE.TOKEN.PAGE_TYPE] = pageType;
    toStore[BG_SCRAPE_TYPE.TOKEN.RECORDS] = records;
    
    chrome.storage.local.set({ [key]: toStore });
  },

  kickoffBackgroundScrapingAsNeeded: function() {
    chrome.runtime.sendMessage({ 
      actionType: MSGTYPE.TOBACKGROUND.LETS_SCRAPE
    });
  },

  cacheBgScrapeUrlKeyFromUrl: function(url) {
    const parsedUrl = URLPARSE.parseUrl(url);

    if (!parsedUrl) {
      return;
    }

    switch (parsedUrl.pageType) {
      case PAGETYPE.TWITTER.TWEETS:
        BGFETCH_REQUEST.TWITTER.TWEETS.cacheBgScrapeUrlKeyFromTweetUrl(url);
        break;
      default:
        break;
    }
  },

  TWITTER: {

    TWEETS: {
      
      cacheBgScrapeUrlKeyFromTweetUrl: function(tweetFullUrl) {
        const parsedUrl = URLPARSE.parseUrl(tweetFullUrl);
        if (!parsedUrl || parsedUrl.pageType != PAGETYPE.TWITTER.TWEETS) {
          return;
        }

        const urlKey = STR.makeTweetRelativeUrl(parsedUrl.owner, parsedUrl.threadDetailId);
        BGFETCH_REQUEST.cacheBgRequest([urlKey], PAGETYPE.TWITTER.TWEETS);
      },
      
      cacheTweetUrlKeysForHasMoreBgScrape: function(tweets) {
        
        if (!_recordingContext || _recordingContext.state != SETTINGS.RECORDING.STATE.AUTO_SCROLL || !_recordingContext.auto || !_recordingContext.auto.resolvesThreads) {
          return false;
        }
        
        let urlKeys = [];
        for (let i = 0; i < tweets.length; i++) {
          let tweet = tweets[i];
          if (tweet.hasMore == true) {
            urlKeys.push(tweet.urlKey);
          }
          if (tweet.quoteTweet && tweet.quoteTweet.urlKey && tweet.quoteTweet.hasMore == true && !URLPARSE.isVirtualQuoteUrlKey(tweet.quoteTweet.urlKey)) {
            urlKeys.push(tweet.quoteTweet.urlKey);
          }
        }

        urlKeys = ES6.distinctify(urlKeys);
        if (urlKeys.length > 0) {
          BGFETCH_REQUEST.cacheBgRequest(urlKeys, PAGETYPE.TWITTER.TWEETS);
          return true;
        }
        else {
          return false;
        }
      }
    },
    
    PROFILES: {

      // cache the request
      cacheTwitterHandlesForProfileScrape: function(content) {
        const handles = BGFETCH_REQUEST.TWITTER.PROFILES.getTwitterHandlesForProfileScrape(content);

        if (!handles || handles.length === 0) {
          return;
        }

        BGFETCH_REQUEST.cacheBgRequest(handles, PAGETYPE.TWITTER.PROFILE);
      },

      // content is a set of lines, where each is either 
      // (a) a simple handle, or
      // (b) the type of record we see in a 'TwitterPsnScore' file, i.e.
      // @BillJones |>>| 16
      // For case (b) a minimum score applies
      getTwitterHandlesForProfileScrape: function(content) {
        let handles = [];
        const cutoff = SETTINGS.getMinProfileFetchScore();

        let rows = content.split('\n');
        for (let r = 0; r < rows.length; r++) {
          let row = rows[r];

          if (row.length > 0) {
            let parts = row.split(FLAT_RDF_SO_DELIM);
            let include = false;

            if (parts.length === 1) {
              include = true;
            }
            else if (parts.length === 2) {
              let score = parseInt(parts[1]);
              if (score && score >= cutoff) {
                include = true;
              }
            }

            if (include) {
              handles.push(parts[0]);
            }
          }
        }

        return handles;
      }

    }
  }  
};