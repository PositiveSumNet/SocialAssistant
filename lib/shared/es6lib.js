...
var ES6 = {
  ...
  // stackoverflow.com/questions/41115702/hide-null-values-in-output-from-json-stringify
  asPrettyJson: function(data) {
    if (!data) { return null; }
    return JSON.stringify(data, ES6.jsonReplacer, 2);
  },
  ...
  // builtin.com/software-engineering-perspectives/javascript-sleep
  // call using await
  ...
  // stackoverflow.com/questions/57618344/how-can-i-download-a-video-given-its-url-in-javascript
  downloadMediaFile: function(cdnUrl, fileName) {
  ...
  // videoDataUri is e.g. "data:video/mp4;base64,AAAAHGZ0eXBtcDQyAAA...""
  swapImgForVideo: function(mediaParent, imgElm, videoDataUri) {
    const videoElm = document.createElement('video');
    videoElm.setAttribute('autoplay', '');
    videoElm.setAttribute('controls', '');
    videoElm.src = videoDataUri;
    imgElm.remove();
    mediaParent.appendChild(videoElm);
    return videoElm;
  },

  preventDragBehaviors: function(e) {
    e.preventDefault();
    e.stopPropagation();
  },
  ...
  isElmaWithinElmB: function(elmA, elmB, selfCheck = true) {
    ...
    while (elmA.parentNode) {
      elmA = elmA.parentNode;
      if (ES6.isElementNode(elmA) && elmA === elmB) {
        return true;
      }
    }
    return false;
  ...
  findUpClass: function(el, cls, selfCheck = true) {
    ...
    while (el.parentNode) {
      el = el.parentNode;
      if (ES6.isElementNode(el) && el.classList.contains(cls)) {
        return el;
      }
    }
    return null;
  ...
  findUpTag: function(el, tag, selfCheck = true) {
    ...
    while (el.parentNode) {
      el = el.parentNode;
      if (ES6.isElementNode(el) && STR.sameText(el.tagName, tag)) {
        return el;
      }
    }
    return null;
  ...
  // get next item in array after hitting the marker value
  getNext: function(arr, marker, caseInsensitive = true) {
    let hit = false;
    for (let i = 0; i < arr.length; i++) {
      // the || is to also allow for non-string comparisons to be evaluated as intended
      if (arr[i] == marker || STR.sameText(arr[i], marker, caseInsensitive)) {
        hit = true;
      }
      else if (hit == true) {
        return arr[i];
