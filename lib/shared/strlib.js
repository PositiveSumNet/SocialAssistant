var STR = {
  ...
  looksLikeCdnRestrictedImg: function(imgUrl) {
    if (!imgUrl) { return false; }
    // twitter images give ERR_BLOCKED_BY_RESPONSE.NotSameOriginAfterDefaultedToSameOriginByCoep when rendered outside of twitter http context
    return imgUrl.indexOf('twitter') > -1 || imgUrl.indexOf('twimg');
  },

  makeMastodonAccountUrl: function(userName, serverDomain) {
    userName = STR.ensurePrefix(userName, '@');
    return `https://${serverDomain}/${userName}`;
  },
