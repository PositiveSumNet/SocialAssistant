var STR = {
  ...
  mastodonAccountFromProfileUrl: function(url) {
    if (!url) { return undefined; };
    if (url.length === 0) { return ''; }

    const splitUrl = STR.splitMastodonUrl(url);
    const fullHandle = STR.standardizeMastodonAccount(splitUrl.handleOnly, splitUrl.server);
    return fullHandle;
  },

  makeMastodonProfileUrl: function(fullHandle, server) {
    server = STR.ensurePrefix(server, 'https://');
    fullHandle = STR.ensurePrefix(fullHandle, '@');
  ...
  makeProfileUrl: function(fullHandle, site) {
    switch(site) {
      case SITE.MASTODON:
        return makeMastodonProfileUrl(fullHandle, site);
      case SITE.TWITTER:
      default:
