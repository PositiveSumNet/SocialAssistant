var STR = {
  ...
  MASTODON: {
    ...
    makeProfileUrl: function(fullHandle, linkViaUserHomeMastodonServer) {
      ...
      if (linkViaUserHomeMastodonServer && linkViaUserHomeMastodonServer != server) {
    ...
    splitUrl: function(url) {
      ...
      let server = STR.stripSuffix(STR.stripHttpWwwPrefix(urlParts[0]),'/');
      let handleOnly = STR.ensurePrefix(urlParts[1], '@');
      
      if (urlParts[1].indexOf('@') > 0) {
        // scenario (b)
        const resplit = urlParts[1].split('@');
        server = resplit[1];
        handleOnly = STR.ensurePrefix(resplit[0], '@');
      }
