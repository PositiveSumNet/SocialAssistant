...
var RECORDING = {
  ...
  storeThreadExpansionUrlKeyAsNeeded: async function(post) {
    if (post.hasMore == true && STR.hasLen(post.threadUrlKey)) {
      await SETTINGS.RECORDING.saveThreadExpansionUrlKey(post.threadUrlKey);
    }
  },
  ...
  onSavedThreadDetailPage: function(posts) {
    ...
    const activePageUrlKey = STR.makeTweetRelativeUrl(parsedUrl.owner, parsedUrl.threadDetailId);
    const threadUrlKeys = ES6.distinctify(posts.filter(function(r) { return STR.hasLen(r[SAVABLE_TWEET_ATTR.threadUrlKey]); }).map(function(r) { return r[SAVABLE_TWEET_ATTR.threadUrlKey]; }));

    if (threadUrlKeys.length == 0) { return; }

    const matchedThreadUrlKey = threadUrlKeys.find(function(k) { return STR.sameText(k, activePageUrlKey); });

    if (!STR.hasLen(matchedThreadUrlKey)) { return; }
    // background needs to know it's fair-game to nuke the thread expansion key
    chrome.runtime.sendMessage({
      actionType: MSGTYPE.TOBACKGROUND.SAVED_THREAD, 
      threadUrlKey: matchedThreadUrlKey
    });
