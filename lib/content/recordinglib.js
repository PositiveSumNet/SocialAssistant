...
var RECORDING = {
  ...
  storeThreadExpansionUrlKeyAsNeeded: function(post) {
  ...
  // similar pattern as storeThreadExpansionUrlKeyAsNeeded
  storeExtractableVideoUrlKeyAsNeeded: function(savable) {
    // handles either SAVABLE_TWEET_ATTR or TWEET_POST_IMG_ATTR
    if (STR.hasLen(savable.urlKey) && STR.isTruthy(savable.embedsVideo) || STR.isTruthy(savable.isEmbeddedVideo)) {
      chrome.runtime.sendMessage({
        actionType: MSGTYPE.TOBACKGROUND.FOUND_EMBEDDED_VIDEO, 
        urlKey: savable.urlKey
      });
    }
  },
