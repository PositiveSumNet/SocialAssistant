...
// background
var _isBackgroundRecordingUrl = false;
var _doneRecording = false;
...
var RECORDING = {
  ...
  calcShouldAutoScroll: function() {
    return _isBackgroundRecordingUrl || _recordingContext.state == SETTINGS.RECORDING.STATE.AUTO_SCROLL;
  },
  ...
  closeSelf: function() {
    // background page is done; nav to a no-op page
    chrome.runtime.sendMessage({
      actionType: MSGTYPE.TO_OFFSCREEN.CLOSE_FRAME,
      url: document.location.href});
},

  saveAsNeeded: function(closeOnSaveComplete) {
    if (_savables.length > 0) {
      ...
      function(response) {
        ...
        if (response && response.success === true && response.saved && response.saved.length > 0) {
          ...
          if (_doneRecording == true && _isBackgroundRecordingUrl == true) {
            BGFETCH_REQUEST.logToBackground('complete A: ' + document.location.href);
    ...
    else if (_isBackgroundRecordingUrl == true) {
      BGFETCH_REQUEST.logToBackground('complete B: ' + document.location.href);
      BGFETCH_REQUEST.onCompleteBgScrape();
  ...
  scrollAsNeeded: function(avoidScrollIfHidden) {
    ...
    if (scrollable === true) {
      ...
      if (!throttled && _emptyScrollCount > 3 && isHidden == false) {
        ...
        if (navAnchor) {
          if (_isBackgroundRecordingUrl == true) {
            // we want to get to the next page of detail, but in a separate offscreen attempt
            BGFETCH_REQUEST.cacheBgScrapeUrlKeyFromUrl(navAnchor.href);
          }
          else {
