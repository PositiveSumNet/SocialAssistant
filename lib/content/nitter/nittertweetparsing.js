var NEETPARSE = {
  ...
  buildTweetFromElm: function(tweetElm, parsedUrl) {
    ...
    if (card) {
      tweet.card = card;
      tweet.card.urlKey = tweet.urlKey;
    ...
    if (!parsedUrl.threadDetailId) {
      tweet.hasMore = threadInfo[THREAD_INFO.unfurlRequired] || NEETPARSE.getThreadHasMore(tweetElm, postText, tweet.urlKey);
    }

    RECORDING.infuseImgCdns(tweet, imgUrls);
    NEETPARSE.attachStats(tweet, tweetElm);
    
    return tweet;
  },

  getStat: function(tweetElm, iconSelector) {
    const iconElm = tweetElm.querySelector(iconSelector);
    if (!iconElm) { return null; }
    const iconContainer = ES6.findUpClass(iconElm, 'icon-container');
    if (!iconContainer) { return null; }
    const stat = parseInt(iconContainer.innerText.trim());
  ...
  attachStats: function(tweet, tweetElm) {
    if (!tweetElm) { return; }
    const replyCount = NEETPARSE.getStat(tweetElm, '.tweet-stat .icon-comment');
    const likeCount = NEETPARSE.getStat(tweetElm, '.tweet-stat .icon-heart');
    const rtCount = NEETPARSE.getStat(tweetElm, '.tweet-stat .icon-retweet');
    const qtCount = NEETPARSE.getStat(tweetElm, '.tweet-stat .icon-quote');

    if (replyCount) {
      tweet[SAVABLE_TWEET_ATTR.replyCount] = replyCount;
    }
    if (likeCount) {
      tweet[SAVABLE_TWEET_ATTR.likeCount] = likeCount;
    }
    // need to add them together to be comparable to what we're able to obtain via twitter
    const reshareCount = (rtCount || 0) + (qtCount || 0);
    if (reshareCount > 0) {
      tweet[SAVABLE_TWEET_ATTR.reshareCount] = reshareCount;
    }
  },
  ...
  getPostUrlKeyForQuoteTweet: function(qtTweetElm) {
    ...
    let href = STR.cleanTweetUrlKey(anchor.getAttribute('href'));
    return href;
  },
