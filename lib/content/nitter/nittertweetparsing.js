var NEETPARSE = {
  ...
  buildTweetFromElm: function(tweetElm) {
    ...
    tweet.authorCdnImgUrl = NEETPARSE.getAuthorCdnImgUrl(tweetElm);
    ...
    // for twitter, this is a separate recording
    tweet.postImgs = NEETPARSE.getTweetImageInfos(tweetElm, tweet.urlKey);
  ...
  getQuotedTweet: function(tweetElm) {
  ...
  getThreadHasMore: function(tweetElm) {
    const showMoreAnchor = tweetElm.querySelector('.show-thread');
    return showMoreAnchor && showMoreAnchor.innerText.toLowerCase().indexOf('show') > -1;
  },

  getTweetImageInfos: function(tweetElm, postUrlKey) {
    
    const infos = [];
    
    // queued up to play (depends on nitter settings)
    const imgPlayableVideos = Array.from(tweetElm.querySelectorAll('.attachments .video-container video'));
    ...
    const imgShuntedVideos = Array.from(tweetElm.querySelectorAll('.attachments .video-container img'));
    ...
    const imgSansVideos = Array.from(tweetElm.querySelectorAll('.attachments img'));
