var NEETPARSE = {
  ...
  buildTweetFromElm: function(tweetElm, parsedUrl) {
    ...
    if (!parsedUrl.threadDetailId) {
      tweet.hasMore = NEETPARSE.getThreadHasMore(tweetElm, tweet.urlKey);
    }
  ...
  THREADING: {
    ...
    getThreadInfo: function(tweetElm, parsedUrl, tweetUrlKey) {
      if (!parsedUrl.threadDetailId) {
        // on timeline stream
        return NEETPARSE.THREADING.getMainStreamThreadInfo(tweetElm);
      }
      else {
    ...
    getMainStreamThreadInfo: function(tweetElm) {
    ...
    getPostPageThreadInfo: function(tweetUrlKey) {
      ...
      for (let i = 0; i < tweetElms.length; i++) {
        ...
        let thisUrlKey = NPARSE.getTweetRelativeUrl(thisElm);
