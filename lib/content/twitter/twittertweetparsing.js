var TWEETPARSE = {
  ...
  buildTweetCard: function(imgElm) {
    ...
    card.cardText = TWEETPARSE.extractCardText(anchor);
  ...
  THREADING: {
    ...
    getStreamThreadInfo: function(tweetElm, parsedUrl, tweetUrlKey) {
      ...
      if (!STR.hasLen(threadUrlKey)) {
        // fallback to the tweet url *unless* this is the 'home' context, in which case we lack
        // sufficient context to be sure
        if (parsedUrl.originalPageType != PAGETYPE.TWITTER.HOME) {
