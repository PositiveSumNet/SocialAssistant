var TWEETPARSE = {
  ...
  buildTweetFromElm: function(tweetElm) {
    ...
    const quoted = TWEETPARSE.getQuotedTweet(tweetElm);
    if (quoted) {
  ...
  getQuotedTweet: function(tweetElm) {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (candidate.innerText == 'Quote Tweet') {
        ...
        if (sibl && sibl.getAttribute('tabindex') == '0') {
          qtElm = sibl;
          break;
        }
      }
  ...
  getParentTweetElmInfo: function(el) {
    ...
    while (el.parentNode && el.parentNode.nodeType == 1 /* element */) {
      ...
      else if (el.getAttribute('tabindex', '0') && el.previousSibling && el.previousSibling.innerText == 'Quote Tweet') {
        // a quote tweet
        return {
          elm: el,
          quoted: true
  ...
  getPostUrlKeyFromImg: function(imgElm) {
    const tweetElmInfo = TWEETPARSE.getParentTweetElmInfo(imgElm);
