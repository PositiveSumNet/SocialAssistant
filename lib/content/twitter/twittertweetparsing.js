var TWEETPARSE = {
  ...
  getReplyToUrlKey: function(tweetElm) {
    ...
    const timestampElm = TWEETPARSE.getTimestampElm(authorHeadlineElm, tweetElm);
    const urlKey = TWEETPARSE.getTweetRelativeUrl(timestampElm, tweetElm);
  ...
  THREADING: {
    ...
    getThreadInfo: function(tweetElm, parsedUrl, tweetUrlKey) {
      if (!parsedUrl.threadDetailId) {
        // on timeline stream
        return TWEETPARSE.THREADING.getMainStreamThreadInfo(tweetElm, tweetUrlKey);
      }
      else {
    ...
    getMainStreamThreadInfo: function(tweetElm, tweetUrlKey) {
      const response = {};
      ...
      // (on the main page we only expect a couple shown, let alone 10)
      ...
      for (let i = 0; i < 10; i++) {
        ...
        else if (TWEETPARSE.THREADING.isMoreRepliesDiv(container) == true) {
        ...
        else if (!STR.hasLen(thisTweetElm.innerText)) {
