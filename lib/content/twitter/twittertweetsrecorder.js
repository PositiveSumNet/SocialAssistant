...
var TWEETSREC = {
  ...
  processTweetPostImages: function(imgElms) {
    ...
    for (let i = 0; i < imgElms.length; i++) {
      ...
      let pair = TWEETPARSE.buildTweetPostImgCdnUrlPair(imgElm);
      if (pair) {
        urlPairs.push(pair);
      }
    }
    ...
    for (let mutation of mutations) {
      if (mutation.type === 'childList') {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          // tweets
          let tweetElms = Array.from(TPARSE.getTweetElms(node));
          if (tweetElms && tweetElms.length > 0) {
            TWEETSREC.processTweets(tweetElms);
          }

          // author img urls
          let authorImgElms = Array.from(TPARSE.getTweetAuthorImgElms(node));
          if (authorImgElms && authorImgElms.length > 0) {
            TWEETSREC.processAuthorImages(authorImgElms);
          }

          // tweet post img urls
          let tweetPostImgElms = Array.from(TPARSE.getTweetPostImgElms(node));
