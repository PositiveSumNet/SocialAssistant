  ...
  parseTopics: function(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('# ')) {
        ...
        if (topic.Name == 'Authoritarianism') {
          topic.Name = 'Authoritarianism_deprecated';
        }
        topics.push(topic);
      }
        ...
        let words = subParts[1].split(', ').map(function(p) { return p == 'qanon' ? 'qanon_deprecated' : p.trim(); });
        subtopic = {
          Name: subParts[0].trim(),
          Keywords: words
        };
        if (subtopic.Name == 'KeyElections') {
          subtopic.NAme = 'KeyElections_deprecated';
        }
