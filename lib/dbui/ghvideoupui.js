...
var GHVIDEOUP_UI = {
  ...
  onUploadVideoSuccess: function(fileName) {
    document.getElementById('uploadSuccess').classList.remove('d-none');
    document.querySelector('#uploadSuccess span').textContent = fileName;
  ...
  uploadVideoWorker: async function(b64Data, fileName) {
    ...
    const encodedContent = STR.stripPrefix(b64Data, VIDEO_DATA_URL_PREFIX);
    ...
    if (STR.hasLen(existingSha)) {
      ...
      if (existingSha == uploadableSha) {
        // no need to reupload identical content
