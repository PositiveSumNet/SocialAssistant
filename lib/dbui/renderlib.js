  ...
  POST: {
    ...
    TAGGING: {
      ...
      renderTagControl: function(post) {
        ...
        const vals = [];
        vals.push(CMB_NONE_SEL_TEXT.TAG_RATE);
        vals.push(..._topicTags);
        let optsHtml = '';

        for (let i = 0; i < vals.length; i++) {
          let val = vals[i];
          optsHtml = STR.appendLine(optsHtml, `<option value='${i - 1}'>${val}</option>`);
        }

        let starsHtml = '';
        for (let i = 0; i < 5; i++) {
          starsHtml = STR.appendLine(starsHtml, `<a href='#' class='postStar-${i + 1}'><i class='bi-star'></i></a>`);
        }

        // todo: initialize using post based on (a) already saved data or (b) guessing

        const cmbHtml = `
        <span class='postScoredTagger noneSelected'>
          <select class='postTagger' data-toggle='tooltip' title='Important posts are worth tagging...'>
            ${optsHtml}
          </select>
          <span class='postTagStars' data-toggle='tooltip' title='Choose a tag from the dropdown at left, then use the star picker to rate within that category.'>
            ${starsHtml}
          </span>
        </span>
        `;
