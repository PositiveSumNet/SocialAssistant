var RENDER = {
  ...
  renderPerson: function(person, context, renderAnchorsRule, filtered) {
    ...
    if (linkImage) {
      const mdonLinkCls = site === SITE.MASTODON ? ` class='${CLS_MASTODON_ACCOUNT_LINK}'` : '';
  ...
  includesRenderedMastodonLink: function(html) {
    return html.indexOf(CLS_MASTODON_ACCOUNT_LINK) > -1;
  },
  ...
  renderFollowOnePerson: function(person) {
    ...
    let cls = CLS_MASTODON_ACCOUNT_LINK;
    let openTag = `<a href='#'`;
    let closeTag = '</a>';
    let text = 'Follow';
    const sourceIdTag = RENDER.writeSourceIdTag(person);
    ...
    if (person.ImFollowingOnMdon == true) {
      ...
      cls = `${cls} text-mastodon`;
    }

    const tip = `data-toggle='tooltip' title='${text} on Mastodon'`;

    return `${openTag} class='${cls}' style='float:right;display:block;' ${tip} ${sourceIdTag}><i class='bi-mastodon'></i> ${text}${closeTag}`;
  },
  ...
  renderMastodonAnchor: function(display, handle, domain) {
      ...
      return `<a href='${url}' target='_blank' class='${CLS_MASTODON_ACCOUNT_LINK}'>${display}</a>`;
  },
