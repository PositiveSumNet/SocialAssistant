var RENDER = {
  ...
  renderPerson: function(person, context, renderAnchorsRule, filtered) {
    ...
    if (avatarOnly) {
      ...
      let sourceIdAttr = (hasMastodon) ? RENDER.writeSourceIdTag(person) : '';

      return `<div class='person row striped pt-1' ${roleInfo} ${sourceIdAttr}>
  ...
  writeSourceIdTag: function(person) {
    if (person.SourceId && person.SourceId.length > 0) {
      const site = APPGRAPHS.getSiteFromGraph(person.NamedGraph);
      if (site === SITE.MASTODON) {
        return `${MASTODON.TAG.SOURCE_ID}='${person.SourceId}'`;
      }
    }
  ...
  renderFollowOnePerson: function(person) {
    ...
    const tip = `data-toggle='tooltip' title='${text} on Mastodon'`;

    return `${openTag} class='${cls}' style='float:right;display:block;' ${tip}><i class='bi-mastodon'></i> ${text}${closeTag}`;
  },
