var RENDER = {
  ...
  renderPerson: function(person, context, renderAnchorsRule, filtered) {
    ...
    if (linkImage) {
      const mdonLinkCls = site === SITE.MASTODON ? ` class='${CLS.MASTODON_ACCOUNT_LINK}'` : '';
    ...
    if (avatarOnly) {
      ...
      const hasMastodon = RENDER.includesRenderedMastodonLink(renderedHandle) || RENDER.includesRenderedMastodonLink(img) || RENDER.includesRenderedMastodonLink(detail);
      const followOne = (hasMastodon) ? RENDER.renderFollowOnePerson(person) : '';
  ...
  includesRenderedMastodonLink: function(html) {
    return html.indexOf(CLS.MASTODON_ACCOUNT_LINK) > -1;
  },

  renderFollowOnePerson: function(person) {
    
    let cls = '';
    let openTag = `<a href='#'`;
    let closeTag = '</a>';
    let text = 'Follow';
    let tip = `data-toggle='tooltip' title='${text} on Mastodon'`;

    if (person.ImFollowingOnMdon == true) {
      cls = `text-success ${CLS.MASTODON_FOLLOWING_ALREADY}`;
      openTag = '<div';
      closeTag = '</div>';
      text = 'Following';
    }
      ...
      const mdonPendingFollowStatus = SETTINGS.getPendingFollowAppearsAccepted(SITE.MASTODON, person.Handle);

      if (mdonPendingFollowStatus === true) {
        cls = `${CLS.MASTODON_FOLLOW_LIKELY_ACCEPTED}`;
        openTag = '<div';
        closeTag = '</div>';
        text = 'Pending';
        tip = `data-toggle='tooltip' title='Pending confirmation (will update when you re-download connections).'`;
      }
      else if (mdonPendingFollowStatus === false) { 
        cls = `${CLS.MASTODON_FOLLOW_AWAITING_RESPONSE}`;
        openTag = '<div';
        closeTag = '</div>';
        text = 'Request submitted';
        tip = `data-toggle='tooltip' title='Request submitted'`;
      }
      else {
        // undefined -- there is no pending follow
        cls = `${CLS.MASTODON_FOLLOW_ONE} text-mastodon`;
      }
    }

    return `${openTag} class='${cls}' style='float:right;display:block;' ${tip}><i class='bi-mastodon'></i> <span class='follow-one-text'>${text}</span>${closeTag}`;
  },
  ...
  renderMastodonAnchor: function(display, handle, domain) {
    ...
    return `<a href='${url}' target='_blank' class='${CLS.MASTODON_ACCOUNT_LINK}'>${display}</a>`;
  },
