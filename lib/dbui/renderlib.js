var RENDER = {
  ...
  POST: {
    renderPostAuthor: function(post, site) {

      let handle = DOMPurify.sanitize(post.AuthorHandle);
      handle = STR.ensurePrefix(handle, '@');
      let displayName = DOMPurify.sanitize(post.AuthorName || handle);
      ...
      <span class='postAuthor'>
        <a class='postAuthorLink' href='${profileUrl}' target='_blank'>
          <img class='postAuthorImg' alt='${handle}' src='${imgSrc}' style='width:46px;height:46px;'/>
          <span class='postAuthorName'>${displayName}</span>
          <span class='nameHandleSep'> | </span>
          <span class='postAuthorHandle'>${handle}</span>
        </a>
      </span>
      `;
    },
    ...
    renderReplyTo: function(post) {
      if (!post.ReplyToTweet) { return ''; }

      console.log(post.ReplyToTweet);
      let handle = DOMPurify.sanitize(post.ReplyToTweet.AuthorHandle);
      handle = STR.ensurePrefix(handle, '@');
      let displayName = DOMPurify.sanitize(post.ReplyToTweet.AuthorName || handle);
      displayName = RENDER.prepareDisplayText(displayName, false);

      let postText = STR.ellipsed(post.ReplyToTweet.PostText, 50);
      ...
      <div class='replyToPost'>
        <div class='replyToAuthor'>
          ...
          <span class='replyToAuthorName'>${displayName}</span>
          <span class='nameHandleSep'> | </span>
          <span class='replyToAuthorHandle'>${handle}</span>
        </div>
    ...
    renderPostImage: function(imgCdnUrl, img64Url) {
      ...
      // to-do: change width/height
      return `<div class='postContentImg'><img alt='src-img' src='${imgSrc}' style='max-width:100px;max-height:100px;'/></div>`;
    },
    ...
    renderPostContent: function(post) {
      ...
      if (post.Images) {
        for (let i = 0; i < post.Images.length; i++) {
          ...
          regImgsHtml = `
          ${regImgsHtml}
          ${RENDER.POST.renderPostImage(regImg.RegImgCdnUrl, regImg.RegImg64Url)}
          `;
        }
      }
