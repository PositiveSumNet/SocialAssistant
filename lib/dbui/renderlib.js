  ...
  POST: {
    ...
    TAGGING: {
      ...
      clickRequestNewTopic: function(event) {
        ...
        if (input != null) {
          if (input.indexOf(TOPICS.TOPIC_SUBTOPIC_COLON) > -1) {
            ...
            RENDER.POST.TAGGING.renderNewlyAddedTopics([dummyTopic]);
      ...
      clickDeprecateSubtopicBtn: function(event) {
        ...
        const topicName = RENDER.POST.TAGGING.getClickedTopicName(delBtnElm);
        const parentLi = ES6.findUpClass(delBtnElm, 'subTopicBtn');
        const subtopicName = parentLi.textContent;
      ...
      clickDeprecateTopicBtn: function(event) {
        ...
        const topicName = RENDER.POST.TAGGING.getClickedTopicName(btn);
      ...
      getClickedTopicName: function(elm) {
      ...
      onDeprecation: function() {
        ...
        Array.from(document.querySelectorAll('.topicListItem')).forEach(function(topicLiElm) {
          let topicName = RENDER.POST.TAGGING.getClickedTopicName(topicLiElm);
          if (STR.hasLen(topicName) && hiddenTopics.has(topicName)) {
            topicLiElm.classList.add('d-none');
          }
          else {
            Array.from(topicLiElm.querySelectorAll('.subTopicBtn')).forEach(function(subtopicLi) {
              let subtopicName = subtopicLi.textContent.trim();
              let concat = TOPICS.concatTopicFullName(topicName, subtopicName);
              if (hiddenConcats.has(concat)) {
                subtopicLi.classList.add('d-none');
              }
            });
          }
        });
      ...
      clickSubTopicAnchor: function(event) {
        ...
        const topicName = RENDER.POST.TAGGING.getClickedTopicName(elm);
      ...
      renderTagControl: function(topicRating) {
        const vals = [];
        const defaultText = topicRating ? TAGGING.CONSTANTS.UNTAG_THIS_POST : TAGGING.CONSTANTS.TAG_RATE;
