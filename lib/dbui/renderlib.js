  ...
  POST: {
    ...
    TAGGING: {
      ...
      clickClearSelection: function(event) {
        ...
        TAGGING.POSTS.setRatingWorker(postScoredTaggerElm, undefined, '');
      ...
      clickRequestNewTopic: function(event) {
        ...
        if (input != null) {
          if (input.indexOf(':') > -1) {
            alert('Topic cannot include the ":" character');
          }
          else {
      ...
      clickRegularTopicAnchor: function(event) {
        ...
        console.log(topicA.textContent);
        return false;
      },

      renderNewlyAddedTopics: function(topics) {
        ...
        for (let i = 0; i < pickerElms.length; i++) {
          ...
          for (let j = 0; j < topics.length; j++) {
            let topic = topics[j];
            RENDER.POST.TAGGING.renderTopicChoice(pickerElm, topic, true, undefined, addAnotherElm);
          }
        }
      },
      ...
      renderSubtopic(subTopicsUl, subTopicName) {
      ...
      renderTopicAnchor: function(topicDiv, topic, specialOnClick, iconCls) {
        ...
        if (topic.Subtopics) {
          for (let i = 0; i < topic.Subtopics.length; i++) {
            ...
            RENDER.POST.TAGGING.renderSubtopic(subTopicsUl, subTopic.Name);
          }
        }
      ...
      getCurrentTopic: function(postScoredTaggerElm, topics) {
        const currentValue = postScoredTaggerElm.getAttribute('data-testid');
        if (!STR.hasLen(currentValue)) { return null; }
        const parts = currentValue.split(TOPICS.TOPIC_SUBTOPIC_COLON);
      ...
      renderTopicChoice: function(topicsUl, topic, isSelected, specialOnClick, insertBeforeElm, iconCls) {
        const topicLi = document.createElement('li');
        topicLi.classList.add('position-relative');
        // the removal btn
        const nukeA = document.createElement('a');
        nukeA.classList.add('btnDeprecateTopic');
        ...
        RENDER.POST.TAGGING.renderTopicAnchor(topicDiv, topic, specialOnClick, iconCls);
      ...
      renderTopicChoices: function(postScoredTaggerElm) {
        ...
        const currentTopic = RENDER.POST.TAGGING.getCurrentTopic(postScoredTaggerElm, topics);
        topicsUl = document.createElement('ul');
        topicsUl.classList.add('topicsPicker');
        topicsUl.classList.add('list-inline');
        let clearSelectionElm = RENDER.POST.TAGGING.renderTopicChoice(topicsUl, TAGGING.CONSTANTS.TAG_REMOVE, false, RENDER.POST.TAGGING.clickClearSelection, undefined, 'bi-eraser');
        clearSelectionElm.classList.add('clearTopicBtn');
        for (let t = 0; t < topics.length; t++) {
          let topic = topics[t];
          if (!TOPICS.isDeprecatedTopic(topic)) {
            let isSelected = currentTopic && STR.sameText(currentTopic.Name, topic.Name);
            RENDER.POST.TAGGING.renderTopicChoice(topicsUl, topic, isSelected);
          }
        }
        let addAnotherElm = RENDER.POST.TAGGING.renderTopicChoice(topicsUl, TAGGING.CONSTANTS.REQUEST_TOPIC, false, RENDER.POST.TAGGING.clickRequestNewTopic, undefined, 'bi-plus-circle');
      ...
      configureTagAndRate: function(postScoredTaggerElm) {
        ...
        cmbElm.addEventListener('change', (event) => {
          TAGGING.POSTS.setRating(postScoredTaggerElm);
        });
        ...
        for (let i = 0; i < starAnchors.length; i++) {
          ...
          starAnchor.onclick = function(event) {
            TAGGING.POSTS.setRating(postScoredTaggerElm, i + 1);
            return false;
          }
        }
      },
      ...
      renderTagControl: function(topicRating) {
        const vals = [];
        const defaultText = topicRating ? TAGGING.CONSTANTS.TAG_REMOVE : TAGGING.CONSTANTS.TAG_RATE;
        vals.push(defaultText);
        vals.push(..._topicTags);
        vals.push(TAGGING.CONSTANTS.REQUEST_TOPIC);
        let optsHtml = '';
        ...
        for (let i = 0; i < vals.length; i++) {
          ...
          let val64 = STR.toBase64(val);
          optsHtml = STR.appendLine(optsHtml, `<option ${selected} value='${val64}'>${val}</option>`);
        }
        ...
        const tipText = 'Exceptional posts are worth tagging. After assigning its topic, you can rate it.';
        ...
        <span class='${clsNoneSel}${clsQuintile}postScoredTagger' data-testid='${postSubtopicValue}'>
          ...
          <select class='postTagger' data-toggle='tooltip' title='${tipText}'>
            ${optsHtml}
          </select>
