var RENDER = {
  ...
  POST: {
    ...
    TAGGING: {
      ...
      setRating: function(postScoredTaggerElm, subtopicName, rating) {
        let oldSubtopicName = postScoredTaggerElm.getAttribute('data-testid');
        let oldRating;
        
        // a) the css and stored values
        // first clear out any existing value
        // *IF* a new rating is provided
        ...
        for (let i = 0; i < clsNames.length; i++) {
          ...
          if (clsName.startsWith('quintile-')) {
            oldRating = parseInt(clsName.split('-')[1]);
        ...
        console.log('for ' + urlKey);
        if (STR.hasLen(newTagName)) {
          console.log('save: ' + newTagName);
        }
        if (STR.hasLen(oldTagName)) {
          console.log('delete: ' + oldTagName);
        }
      },
      ...
      configureTagAndRate: function(postScoredTaggerElm) {
        ...
        cmbElm.addEventListener('change', (event) => {
          ...
          RENDER.POST.TAGGING.setRating(postScoredTaggerElm, subtopicName);
        });
        ...
        for (let i = 0; i < starAnchors.length; i++) {
          ...
          starAnchor.onclick = function(event) {
            const valInt = cmbElm.value;
            if (valInt > -1) {
              const subtopicName = _topicTags[valInt];
              RENDER.POST.TAGGING.setRating(postScoredTaggerElm, subtopicName, i + 1);
            }
            return false;
          }
        }
      ...
      initTopicTags: function() {
        if (_topicTags.length > 0) { return; }  // already initialized
          ...
          if (!TOPICS.isDeprecatedTopic(topic)) {
            for (let s = 0; s < topic.Subtopics.length; s++) {
              ...
              if (!TOPICS.isDeprecatedSubtopic(subtopic)) {
                let concat = `${topic.Name}${TOPICS.RENDER_DELIM}${subtopic.Name}`;
                tags.push(concat);
              }
            }
          }
      ...
      renderTagControl: function(post) {
        RENDER.POST.TAGGING.initTopicTags();

        const vals = [];
        vals.push(CMB_SPECIAL.TAG_RATE);
        vals.push(..._topicTags);
        vals.push(CMB_SPECIAL.REQUEST_TAG);
        let optsHtml = '';

        // todo: finalize the post properties to use for this
        // make sure postSubtopicValue is not undefined
        const postSubtopicValue = post.Subtopic || 'Democracy: Liberalism';
        const postQuintile = 3;
        ...
        // todo: initialize using post based on (a) already saved data or (b) guessing
    ...
    renderPostBody: function(post, site) {
      ...
      const tagggingElm = RENDER.POST.TAGGING.renderTagControl(post);
