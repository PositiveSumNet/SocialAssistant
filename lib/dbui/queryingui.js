  ...
  THREAD: {
    ...
    setOneThreadState: function(threadUrlKey) {
      ...
      if (STR.hasLen(threadUrlKey)) {
        ...
        txtSearch.value = '';
        // clear topic filter
        QUERYING_UI.FILTERS.TOPICS.clearTopicFilterChoice();
      }
      else {
  ...
  FILTERS: {
    ...
    TOPICS: {
      renderTopicFilterChoices: function() {
        let choices = [];
        choices.push(TAGGING.CONSTANTS.TAG_FILTER_BY);
      ...
      clearTopicFilterChoice: function() {
        document.getElementById('cmbTopicFilter').value = -1;
      },
      ...
      renderTopicFilterChoice: function(topicsUlElm, topic) {
        ...
        topicLiElm.appendChild(topicDiv);
        topicDiv.classList.add('topicFilterTopic');
