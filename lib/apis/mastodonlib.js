...
var MASTODON = {
  ...
  parseResponseHeaders: function(response) {
    ...
    _mdonApiCallsRemaining = response.headers.get('x-ratelimit-remaining');
    // when the rate limit will reset
    _mdonApilimitResetAt = response.headers.get('x-ratelimit-reset');
  ...
  downloadConnections: function(direction) {
    ...
    MASTODON.clearErrorMsg();
    fetch(`https://${SETTINGS.getMdonServer()}/api/v1/accounts/${_mdonDownloadConnsFor.SourceId}/${_mdonDownloadDirection}?limit=80`, {
        method: 'GET',
        headers: MASTODON.getUserAuthHeader()
    })
    .then(function(response) { 
      ...
      console.log(MASTODON.parseResponseHeaders(response));
