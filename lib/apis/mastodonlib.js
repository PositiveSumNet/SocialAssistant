...
var MASTODON = {
  ...
  setDownloadConnsFor: function(accountAsPerson) {
    _mdonDownloadConnsFor = accountAsPerson;
    document.getElementById('mdonDownloadFollowingListBtn').textContent = `Get Following List (${accountAsPerson.FollowingCount})`;
    document.getElementById('mdonDownloadFollowersListBtn').textContent = `Get Followers (${accountAsPerson.FollowersCount})`;
  },
  ...
  clearDownloadStatus: function() {
    ...
    document.getElementById('mastodonDownloadDoneIcon').style.display = 'none';
  ...
  updateConnectionDownloadStatus: function(ownerAccountId, pagedPeople, followDirection) {
    ...
    if (_fetchedCtr === outOf) {
      document.getElementById('mastodonDownloadDoneIcon').style.display = 'inline';
    }
  },
  ...
  onGotSavedCount: function(increment, ownerAccountId, followDirection) {
    ...
    if (_savedCtr === outOf) {
      ...
      // let's just show it for a few seconds
      setTimeout(() => {
  ...
  downloadConnections: function(followDirection, isSubsequentPage) {
    ...
    let url = `https://${SETTINGS.getMdonServer()}/api/v1/${api}/${accountId}/${followDirection}?limit=80`;
  ...
  suggestRemoteAccountOwner: function(userInput) {
    if (userInput.length === 0) {
      MASTODON.setDownloadConnsFor(_mdonConnectedUser);
      MASTODON.renderNetwork();
      return;
    }
    else {
      // reset the text
      document.getElementById('mdonDownloadFollowingListBtn').textContent = `Get Following List`;
      document.getElementById('mdonDownloadFollowersListBtn').textContent = `Get Following List`;
    }
  ...
  onChooseRemoteOwner: function() {
    MASTODON.searchRemoteAccounts(txtRemoteMdon.value, MASTODON.onResolveRemoteOwner, 1, 1);
    mdonRemoteOwnerPivotPicker.replaceChildren();
  },

  onResolveRemoteOwner: function(result) {
    if (result && result.length === 1) {
      const person = result[0];
      MASTODON.setDownloadConnsFor(person);
    }
  },
  ...
  searchRemoteAccounts: function(searchText, successCallback, searchLimit, takeLimit) {
    const exactMatch = searchLimit === 1 && takeLimit === 1;
    ...
    let url = `https://${SETTINGS.getMdonServer()}/api/v2/search?type=accounts&q=${searchText}&limit=${searchLimit}`;

    fetch(url, {
      method: 'GET',
      headers: MASTODON.getAppAuthHeader()
    })
    .then(function(response) { 
