...
var MASTODON = {
  ...
  searchRemoteAccounts: function(searchText, successCallback, searchLimit, takeLimit, server) {
  ...
  tryGetImAlreadyFollowingFromRenderedPersonAnchor: function(anchor) {
    const personElm = ES6.findUpClass(anchor, "person");
    if (!personElm) { return undefined; }
    const followingAlready = personElm.querySelector(`.${CLS.MASTODON_FOLLOWING_ALREADY}`);
    return (followingAlready) ? true : false;
  },

  buildFollowAllMastodonAccountsRequest: function() {
    const anchors = Array.from(document.querySelectorAll(`#connListMain .${CLS.MASTODON_ACCOUNT_LINK}`));
    ...
    for (let i = 0; i < anchors.length; i++) {
      ...
      if (!MASTODON.tryGetImAlreadyFollowingFromRenderedPersonAnchor(anchor)) {
  ...
  getPersonElmFromAnchor: function(anchor) {
    return ES6.findUpClass(anchor, "person");
  },

  getFollowOneAnchor: function(anchor) {
    if (anchor.classList.contains(CLS.MASTODON_FOLLOW_ONE)) {
      return anchor;
    }
    else {
      const personElm = MASTODON.getPersonElmFromAnchor(anchor);
      const followOne = personElm.querySelector(`a.${CLS.MASTODON_FOLLOW_ONE}`);
      return followOne;
    }
  },

  listenForFollowButtonClick: function(scope) {
    Array.from(scope.querySelectorAll(`a.${CLS.MASTODON_FOLLOW_ONE}`)).forEach(function(anchor) {
      anchor.onclick = function(event) {
        let personElm = MASTODON.getPersonElmFromAnchor(anchor);
        if (personElm) {
          let accountLink = personElm.querySelector(`.${CLS.MASTODON_ACCOUNT_LINK}`);
          if (accountLink) {
            if (STR.MASTODON.looksLikeImposterProfileUrl(accountLink.href)) {
              MASTODON.setErrorMsg(`Sorry, this isn't a Mastodon account after all.`);
            }
            else {
              let account = STR.MASTODON.accountFromProfileUrl(accountLink.href);
              let request = { handle: account, anchor: anchor };
  ...
  restyleForPendingFollowRequest: function(anchor, appearsAccepted) {
    if (anchor.classList.contains('text-mastodon')) {
      anchor.classList.remove('text-mastodon');
    }

    if (anchor.classList.contains(CLS.MASTODON_FOLLOW_ONE)) {
      anchor.classList.remove(CLS.MASTODON_FOLLOW_ONE);
    }
  
    let text = '';
    if (appearsAccepted) {
      if (!anchor.classList.contains(CLS.MASTODON_FOLLOW_LIKELY_ACCEPTED)) {
        anchor.classList.add(CLS.MASTODON_FOLLOW_LIKELY_ACCEPTED);
      }
      text = 'Pending';
    }
    else {
      if (!anchor.classList.contains(CLS.MASTODON_FOLLOW_AWAITING_RESPONSE)) {
        anchor.classList.add(CLS.MASTODON_FOLLOW_AWAITING_RESPONSE);
      }
      text = 'Request submitted';
    }

    let span = anchor.querySelector('.follow-one-text');
    if (span) {
  ...
  onMadeFollowRequest: function(request) {
    ...
    if (followOneAnchor) {
      MASTODON.restyleForPendingFollowRequest(followOneAnchor, request.following);
    }
  }
};