...
var MASTODON = {
  ...
  getActiveUserId: function() {
    if (!_mdonUser) { return ''; }
    return _mdonUser[PERSON_ATTR.SOURCE_ID] || '';
  },

  getActiveUserHandle: function() {
    if (!_mdonUser) { return ''; }
    return _mdonUser[PERSON_ATTR.HANDLE] || '';
  },

  isConnected: function() {
    return MASTODON.getActiveUserId().length > 0;
  },
  ...
  render: function() {
    ...
    if (MASTODON.getActiveUserId().length > 0 && MASTODON.getActiveUserHandle().length > 0 && _mdonUserAuthToken.length > 0) {
      ...
      mdonReconnectingAs.textContent = `Reconnecting as ${MASTODON.getActiveUserHandle()}...`;
  ...
  accountToPerson: function(account) {
    ...
    const urlParts = account.url.split('@');
    ...
    const fullHandle = STR.standardizeMastodonAccount(handleOnly, server);
  ...
  onReceivedLoggedInUser: function(data) {
    _mdonUser = MASTODON.accountToPerson(data);
    console.log(data);
    console.log('foo');
    console.log(_mdonUser);
    _mdonDownloadConnsFor = MASTODON.accountToPerson(data); // a copy so it's not a shared reference
    ...
    chrome.storage.local.set({ [MASTODON.OAUTH_CACHE_KEY.USER]: _mdonUser });
    // rendering
    document.getElementById('mdonUserAccount').textContent = MASTODON.getActiveUserHandle();
    document.getElementById('txtMdonDownloadConnsFor').value = STR.stripPrefix(MASTODON.getActiveUserHandle(), '@');
    // to pre-populate DB UI owner textbox
    localStorage.setItem(SETTINGS.ownerCacheKey(SITE.MASTODON), MASTODON.getActiveUserHandle());
