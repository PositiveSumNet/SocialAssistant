...
var MASTODON = {
  ...
  tryGetSourceIdFromRenderedPersonAnchor: function(anchor) {
    const personElm = ES6.findUpClass(anchor, "person");
    
    if (!personElm || !personElm.hasAttribute(MASTODON.TAG.SOURCE_ID)) {
      return '';
    }

    return personElm.getAttribute(MASTODON.TAG.SOURCE_ID);
  },
      ...
      if (!MASTODON.tryGetImAlreadyFollowingFromRenderedPersonAnchor(anchor)) {
        if (!STR.MASTODON.looksLikeImposterProfileUrl(anchor.href)) {
          ...
          if (!accountSet.has(account)) {
            let sourceId = MASTODON.tryGetSourceIdFromRenderedPersonAnchor(anchor);
            requests.push({ handle: account, sourceId: sourceId});
        ...
        if (personElm) {
          ...
          if (accountLink) {
            if (STR.MASTODON.looksLikeImposterProfileUrl(accountLink.href)) {
              ...
              let sourceId = MASTODON.tryGetSourceIdFromRenderedPersonAnchor(accountLink);
              let request = { handle: account, sourceId: sourceId};
  ...
  // sourceId was optional (resolved during fetch if missing)
  followWorker: function(requests) {
    for (let i = 0; i < requests.length; i++) {
      ...
      if (request.sourceId && request.sourceId.length > 0) {
        MASTODON.followIdentifiedAccount(request.sourceId);
      }
      else {
        MASTODON.searchRemoteAccounts(request.handle, function(people) {
          if (people.length > 0) {
            MASTODON.followIdentifiedAccount(people[0].SourceId);
          }
        }, 1, 1);
      }
    }
  ...
  followIdentifiedAccount: function(sourceId) {
