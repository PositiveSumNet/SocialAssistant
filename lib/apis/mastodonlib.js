...
var MASTODON = {
  ...
  warnOfApiThrottleAsNeeded: function() {
    if (_mdonApilimitResetAt && _mdonApiCallsRemaining < 3) {
      ...
      const msg = `The Mastodon API is asking to rest until ${dt.toLocaleTimeString()}`;
  ...
  accountToPerson: function(account) {
    ...
    const fullHandle = STR.MASTODON.accountFromProfileUrl(account.url);
  ...
  saveConnections: function(ppl, owner) {

  },

  applyPagingNext: function(url) {
    if (_paging && _paging.next && _paging.next.parmValue && _paging.next.parmValue.length > 0 && STR.sameText(_paging.baseUrl, url)) {
      return `${url}&${_paging.next.parmName}=${_paging.next.parmValue}`;
    }
    else {
      return url;
    }
  ...
  downloadConnections: function(direction) {
    ...
    if (!_mdonDownloadConnsFor || !_mdonDownloadConnsFor.SourceId || _mdonDownloadConnsFor.SourceId.length === 0) {
      MASTODON.setErrorMsg('Owner account is not set.');
    }

    MASTODON.clearErrorMsg();
    MASTODON.warnOfApiThrottleAsNeeded();
    
    let url = `https://${SETTINGS.getMdonServer()}/api/v1/accounts/${_mdonDownloadConnsFor.SourceId}/${_mdonDownloadDirection}?limit=80`;
