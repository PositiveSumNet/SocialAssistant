...
var TWITTER_PROFILE_SAVE_MAPPER = {
  ...
  mapSavableSet: function(records, pageType, graph) {
    ...
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialProfileImgBinary.Name).sogs = img64s;
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialProfileLinkMastodonAccount.Name).sogs = mdons;
    ...
    if (withFollowCounts) {
      APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialFollowerCount.Name).sogs = followersCounts;
