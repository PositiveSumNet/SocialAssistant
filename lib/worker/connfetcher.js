...
var CONNFETCHER = {
  ...
  networkSearch: function(request) {
    ...
    const reciproalEntDefn = PAGETYPE.reciproalEntDefn.pageType);
    ...
    const needMutualityCondition = request.mutual && reciproalEntDefn;
    ...
    const joinList = `${listJoinWord} ${entListMember.Name} ${lst}
    ...
    SELECT DISTINCT ${conn}.${entConn.ObjectCol} AS ${CONNSELECTED.NETWORK_SEARCH.Handle}, 
        ${conn}.${APPSCHEMA.COLUMNS.Timestamp} AS ${CONNSELECTED.NETWORK_SEARCH.Timestamp},
    ...
    LEFT JOIN ${tblDescription} ${descrip} ON ${descrip}.${entDescription.SubjectCol} ${equalsConnObject} 
      AND ${descrip}.${equalsConnGraph}
    LEFT JOIN ${tblImgCdnUrl} ${imgcdn} ON ${imgcdn}.${entImgCdnUrl.SubjectCol} ${equalsConnObject} 
      AND ${imgcdn}.${equalsConnGraph}
    LEFT JOIN ${tblImg64Url} ${img64} ON ${img64}.${entImg64Url.SubjectCol}  ${equalsConnObject} 
