...
var CONNREQUEST = {
  ...
  MASTODON_FOLLOWING: {
    WHERE_IM_FOLLOWING: 'whereImFollowingOnMdon',
    WHERE_IM_NOT_FOLLOWING: 'whereImNotFollowingOnMdon'
  },
  ...
  NETWORK_SEARCH: {
    ...
    withEmail: 'withEmail',
    withUrl: 'withUrl',
    mdonFollowing: 'mdonFollowing',  // CONNREQUEST.MASTODON_FOLLOWING
...
var CONNFETCHER = {
  ...
  MASTODON_CONDITION_WRITER: {
    ...
    queryWriteWorker: function(request, leftJoined, andNotExists, linkToAlias, linkToEntity, conjunction) {
      ...
      result.parms.push({ $myMdon: request.myMastodonHandle });
      const joinWord = leftJoined ? 'LEFT JOIN' : 'JOIN';

      result.joinSql = `${joinWord} ${entMcon.Name} ${mcon} 
      ...
      if (andNotExists) {
        ...
        result.andSql = `  ${conjunction} ${mcon}.${SCHEMA_CONSTANTS.COLUMNS.Timestamp} IS NULL`;
      }

      return result;
    },

    twitterQueryWriteWorker: function(request, leftJoined, andNotExists, conjunction) {
      const entLinkMdon = APPSCHEMA.SocialProfileLinkMastodonAccount;
      return CONNFETCHER.MASTODON_CONDITION_WRITER.queryWriteWorker(request, leftJoined, andNotExists, 'mdon', entLinkMdon, conjunction);
    },

    mdonQueryWriteWorker: function(request, leftJoined, andNotExists, conjunction) {
      const entConn = PAGETYPE.getRootEntDefn(request.pageType);
      return CONNFETCHER.MASTODON_CONDITION_WRITER.queryWriteWorker(request, leftJoined, andNotExists, 'conn', entConn, conjunction);
    },

    twitterQueryWhereImFollowingResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.twitterQueryWriteWorker(request, false, false, conjunction);
    },

    twitterQueryWhereImNotFollowingResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.twitterQueryWriteWorker(request, true, true, conjunction);
    },

    twitterQueryUnconditionedResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.twitterQueryWriteWorker(request, true, false, conjunction);
    },

    mdonQueryWhereImFollowingResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.mdonQueryWriteWorker(request, false, false, conjunction);
    },

    mdonQueryWhereImNotFollowingResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.mdonQueryWriteWorker(request, true, true, conjunction);
    },

    mdonQueryUnconditionedResult: function(request, conjunction) {
      return CONNFETCHER.MASTODON_CONDITION_WRITER.mdonQueryWriteWorker(request, true, false, conjunction);
    },

    twitterQueryResult: function(request, conjunction) {
      switch (request.mdonFollowing) {
        case CONNREQUEST.MASTODON_FOLLOWING.WHERE_IM_FOLLOWING:
          return CONNFETCHER.MASTODON_CONDITION_WRITER.twitterQueryWhereImFollowingResult(request, conjunction);
        case CONNREQUEST.MASTODON_FOLLOWING.WHERE_IM_NOT_FOLLOWING:
    ...
    writeResult: function(request, conjunction) {
      ...
      if (request.pageType === PAGETYPE.MASTODON.FOLLOWING && request.networkOwner === request.myMastodonHandle) {
