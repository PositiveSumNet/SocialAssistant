...
var CONNFETCHER = {
  ...
  SYNC: {
    ...
    getProfiles: function(step) {
      ...
      // handle always starts with @, then comes the alpha character marker, then % for 'like'
      // note re params: marker is not user-input
      const pattern = `@${step[SYNCFLOW.STEP.marker]}%`;
      let parm = {key: '$pattern', value: pattern};
      bind.push(parm);
      const markerFilter = `${disp}.${entDisplay.SubjectCol} LIKE ${parm.key}`;
    ...
    getProfileImg64s: function(step) {
      const site = SYNCFLOW.getSite(step.network);
      const graphFilter = APPGRAPHS.getGraphBySite(site);
      const entImg64 = APPSCHEMA.SocialProfileImgBinary;
      const img64 = 'img64';
      ...
      let parm = {key: '$pattern', value: pattern};
      bind.push(parm);
      const markerFilter = `${img64}.${entImg64.SubjectCol} LIKE ${parm.key}`;

      const sql = `
      SELECT ${img64}.${entImg64.SubjectCol} AS ${SYNC_COL.PROFILE_IMGS.Handle},
        ${img64}.${entImg64.ObjectCol} AS ${SYNC_COL.PROFILE_IMGS.Img64},
        ${img64}.${SCHEMA_CONSTANTS.COLUMNS.Timestamp}
      FROM ${entImg64.Name} ${img64}
      WHERE ${markerFilter}
        AND ${img64}.${SCHEMA_CONSTANTS.COLUMNS.NamedGraph} = '${graphFilter}';
      `;
    ...
    getNetworkFollowings: function(step) {
      const site = SYNCFLOW.getSite(step.network);
      const graphFilter = APPGRAPHS.getGraphBySite(site);
      const entConn = APPSCHEMA.SocialProfileImgBinary;
      const conn = 'conn';
      ...
      if (STR.hasLen(step.marker)) {
        ...
        bind.push(parm);
        markerFilter = `  AND ${conn}.${entConn.SubjectCol} > ${parm.key}`;
      }

      const sql = `;with cte AS (
        SELECT DISTINCT ${conn}.${entConn.SubjectCol} AS ${SYNC_COL.FOLLOWINGS.Handle}
        FROM ${entConn} ${conn}
        WHERE ${conn}.${SCHEMA_CONSTANTS.COLUMNS.NamedGraph} = '${graphFilter}'
        ${markerFilter}
        LIMIT 1
      )
      SELECT ${conn}.${entConn.SubjectCol} AS ${SYNC_COL.FOLLOWINGS.Handle},
        ${conn}.${entConn.ObjectCol} AS ${SYNC_COL.FOLLOWINGS.Following},
        ${conn}.${SCHEMA_CONSTANTS.COLUMNS.Timestamp}
      FROM ${entConn} ${conn}
      JOIN cte c ON c.${SYNC_COL.FOLLOWINGS.Handle} = ${conn}.${SYNC_COL.FOLLOWINGS.Handle}
      WHERE ${conn}.${SCHEMA_CONSTANTS.COLUMNS.NamedGraph} = '${graphFilter}';
      `;
