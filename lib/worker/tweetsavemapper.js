...
var TWEET_SAVE_MAPPER = {
  ...
  mapRecordIntoSet: function(record, set, graph) {
    ...
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostRegImgSourceUrl.Name).sogs.push(...pRegImgSrcUrls);
    APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostRegImgBinary.Name).sogs.push(...pRegImgBinaries);
  ...
  mapSavableSet: function(records, pageType, graph) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (STR.hasLen(imgCdnUrl)) {
        APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostRegImgSourceUrl.Name).sogs.push({s: urlKey, o: imgCdnUrl, g: graph});
      }
      if (STR.hasLen(img64)) {
        APPSCHEMA.SAVING.getSubset(set, APPSCHEMA.SocialPostRegImgBinary.Name).sogs.push({s: urlKey, o: img64, g: graph});
      }
    }
