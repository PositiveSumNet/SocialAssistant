...
var MASTODON_CONN_SAVE_MAPPER = {
  ...
  mapSavableSet: function(records, pageType, graph) {
    ...
    const sourceIds = records.map(function(x) {
      return {s: x.Handle, o: x.SourceId, g: graph};
    });
    ...
    set.getSubset(APPSCHEMA.SocialSourceIdentifier.Name).sogs = sourceIds;
    set.getSubset(APPSCHEMA.SocialFollowerCount.Name).sogs = followerCounts;
