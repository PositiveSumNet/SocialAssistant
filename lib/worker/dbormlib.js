...
var DBORM = {
  ...
  MIGRATION: {
    ...
    migrateDbAsNeeded: function(db, scripts) {
      ...
      if (scripts && scripts.length > 0) {
        let codeVersion = getMaxScriptNumber(scripts);
    ...
    getDbScriptVersion: function(db, appName) {
    ...
    ensureMigrationPrereqs: function(db, appName) {
    ...
    writeUpdateMigrationVersionSql: function(scriptNumber, appName) {
    ...
    writeEnsureImportTablesScript: function(scriptNumber) {
      const sql = `
    ...
    // includes update of Migration.Version
    writeEnsureEntityTablesStep: function(entities, scriptNumber) {
      let sql = '';
  ...
  SAVING: {
    ...
    // Called when asked to save pages of data to the db (transferring from storage)
    // where page.key is the key within storage of the data we're transferring, and
    // page.val was the list of items.
    // This method consolidates into larger batch sizes more appropriate for the DB
    // (whereas storage page size is typically based on a smaller scraped page size).
    groupSavablePages: function() {
