...
var DBORM = {
  ...
  SAVING: {
    ...
    writeImportTime: function(sog) {
      if (sog.t && STR.isValidIsoDate(sog.t)) {
        return `'${sog.t}'`;
      }
      else {
        return `datetime('now')`;
    ...
    execBulkImport: function(db, oneToOne, uid, s, o, g, sogs) {
      ...
      for (let i = 0; i < numBatches; i++) {
        ...
        for (let j = 0; j < batchSogs.length; j++) {
          ...
          let time = DBORM.writeImportTime(sog);
    ...
    execUpsert: function(db, savableSet, arr, uid, tbl, oneToOne, subjectCol, objectCol) {
      if (!arr || arr.length === 0) { return; } // no work to do
      const importTable = DBORM.SAVING.getImportTable(oneToOne);
      ...
      if (savableSet.onlyIfNewer === true) {
        ...
        CASE 
          ...
          WHEN ex.${timeCol} < i.ImportTime THEN 1
          ELSE 0
        END = 1
        `;
      }
    ...
    newSavableSet: function(entityDefns, pageType, onlyIfNewer) {
      const subsets = [];
    ...
    save: function(db, savableSet) {
      ...
      for (let i = 0; i < savableSet.subsets.length; i++) {
        ...
        if (subset.sogs.length > 0) {
          ...
          DBORM.SAVING.execUpsert(db, savableSet, subset.sogs, subset.uid, tableName, entityDefn.OneToOne, entityDefn.SubjectCol, entityDefn.ObjectCol);
        }
      }
  ...
  EXPORT: {
    ...
    // this can evolve to pass in timestamp range for incremental exports
    exportBackup: function(request, allEntities) {
      ...
      const entity = (request.nextEntity) ? allEntities.find(function(e) { return e.Name === request.nextEntity; }) : allEntities[0];
      
      const sql = `
      SELECT  ${entity.SubjectCol} AS ${RDFCOL.s}, 
              ${entity.ObjectCol} AS ${RDFCOL.o}, 
              ${SCHEMA_CONSTANTS.COLUMNS.NamedGraph} AS ${RDFCOL.g}, 
              ${SCHEMA_CONSTANTS.COLUMNS.Timestamp} AS ${RDFCOL.t}
      FROM ${entity.Name}
      ...
      if (thisEntityHasMore) {
        ...
        for (let i = 0; i < allEntities.length; i++) {
          if (allEntities[i].Name === entity.Name && i < allEntities.length - 1) {
            nextEntityName = allEntities[i+1].Name;
            break;
          }
        }
      }
  ...
  IMPORT: {
    ...
    receiveSyncableImport: function(request) {
      _importSyncQueue.enqueue(request);
      DBORM.IMPORT.processSyncableImports();
    },

    processSyncableImports: function(bypassLock) {
      ...
      DBORM.IMPORT.processSyncableImport(activeRequest.json);

      if (!_importSyncQueue.isEmpty) {
        processSyncableImports(true);
      }
      else {
        _processingImportSync = false;
      }
    ...
    processSyncableImport: function(json) {
      const importSet = JSON.parse(json);
      // see result structure in DBORM.EXPORT.exportBackup
      const entity = importSet.entity;
      const rows = importSet.rows;
      console.log(entity);
    }
  }
};
