...
var DBORM = {
  ...
  SAVING: {
    ...
    newSavableSet: function(entityDefns, onlyIfNewer) {
      ...
      for (let i = 0; i < entityDefns.length; i++) {
        subsets.push(DBORM.SAVING.newSavableSubset(entityDefns[i]));
      }
  ...
  EXPORT: {
    ...
    exportBackup: function(request, allEntities) {
      ...
      const callbackMsgType = request.callbackMsgType || MSGTYPE.FROMDB.EXPORT.RETURN_EXPORTED_RESULTS;
      ...
      postMessage({ type: callbackMsgType, payload: resultMsg });
    }
  },
  IMPORT: {
    ...
    processSyncableImport: function(allEntities, json) {
      ...
      const savableSet = DBORM.SAVING.newSavableSet(entDefns, true);
      savableSet.getSubset(entity.Name).sogs = importSet.rows;
