...
var DBORM = {
  ...
  SAVING: {
    ...
    save: function(db, savableSet) {
      ...
      // bulk import
      for (let i = 0; i < savableSet.subsets.length) {
        let subset = savableSet.subsets[i];
        if (subset.sogs.length > 0) {
          DBORM.SAVING.execBulkImport(db, subset.entityDefn.OneToOne, followUid, qMark, qMark, qMark, subset.sogs);
        }
      }
