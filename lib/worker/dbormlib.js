...
var DBORM = {
  ...
  SAVING: {
    ...
    setListMember(listMemberEntDefn, memberObjectValue, graph, remove = false, listName = 'favorite') {
      const nowTime = DBORM.getNowTime(remove);   // will be semantically negated if remove is true
      
      // tbc, $s $o $g are sqlite parameter names
      const sql = `
      REPLACE INTO ${listMemberEntDefn.Name} ( ${listMemberEntDefn.SubjectCol}, ${listMemberEntDefn.ObjectCol}, NamedGraph, Timestamp )
      VALUES ( $s, $o, $g, ${nowTime} );
      `;
      
      const bound = {$s: listName, $o: memberObjectValue, $g: graph};
