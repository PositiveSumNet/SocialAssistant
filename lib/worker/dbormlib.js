...
var DBORM = {
  ...
  MIGRATION: {
    ...
    migrateDbAsNeeded: function(db, scripts) {
      ...
      // SELECT * FROM SocialPostEmbedsVideo WHERE sPostUrlKey = '/DrSpockets/status/1716609924518822392';
      // `, []);
    ...
    writeEnsureEntityTableSql: function(entity) {
      ...
      return `
        ...
        ${SCHEMA_CONSTANTS.COLUMNS.NamedGraph} TEXT NOT NULL COLLATE NOCASE,
        ${SCHEMA_CONSTANTS.COLUMNS.Timestamp} datetime,
        ${ux});
  ...
  SAVING: {
    ...
    postSaveStatusMsg: function(msg) {
      postMessage({ type: MSGTYPE.FROMDB.SAVE_STEP_STATUS_MSG, msg: msg });
    },
    ...
    execSaveSet: function(savableSet) {
      let db = DBORM.getDb();
      try {
    ...
    // for ALL graphs!
    deleteBySubject: function(request, allEntities) {
      const entDefn = allEntities.find(function(e) { return e.Name === request.entDefn; });
      let bind = [];
      bind.push(request.subject);

      const sql = `
      DELETE FROM ${entDefn.Name}
      WHERE ${entDefn.SubjectCol} = ?;
      `;
  ...
  IMPORT: {
    ...
    processSyncableImports: function(allEntities, bypassLock) {
      ...
      if (!bypassLock && _processingImportSync === true) {
        ...
        // ignore this new inbound request from the _worker; we'll get to its request soon enough
        return;
      }
      ...
      if (!_importSyncQueue.isEmpty) {
        ...
        // done for now; wait for a new inbound from the _worker
        _processingImportSync = false;
      }
    },
    ...
    processSyncableImport: function(allEntities, json) {
      ...
      postMessage({ type: MSGTYPE.FROMDB.IMPORT.PROCESSED_UPLOAD_VIDEO_BATCH });
    }
  }
};
