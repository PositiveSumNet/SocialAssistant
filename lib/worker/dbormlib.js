...
var DBORM = {
  ...
  dbExec: function(db, args, writing) {
    ...
    try {
      ...
      if (writing) {
        DBORM.LOGGING.logHtml('Sorry, we hit an error. Try refreshing this page.');
      }
      console.trace();
    }
    finally {
  ...
  MIGRATION: {
    ...
    migrateDbAsNeeded: function(db, scripts) {
      ...
      // let debugRows = DBORM.QUERYING.fetch(`SELECT * FROM SocialProfileImgSourceUrl WHERE sHandle like '@%toad%';`, []);
      // console.log(debugRows);
    },
