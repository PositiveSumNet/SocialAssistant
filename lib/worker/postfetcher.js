...
var POSTFETCHER = {
  ...
  postSearch: function(request) {
    ...
    POSTFETCHER.infuseQuoteTweets(request, rows);
  ...
  infuseQuoteTweets: function(request, rows) {
    const qtKeys = rows
      .filter(function(row) { return STR.hasLen(row[POST_SEL.QuoteOfUrlKey]); } )
      .map(function(row) { return row[POST_SEL.QuoteOfUrlKey]; });
    
    if (qtKeys.length === 0) { return; }
    const qtRequest = { pageType: request.pageType };
    const qtRows = POSTFETCHER.postSearchWorker(qtRequest, qtKeys);
    for (let i = 0; i < rows.length; i++) {
      let row = rows[i];
      let qtKey = row[POST_SEL.QuoteOfUrlKey];
      if (STR.hasLen(qtKey)) {
        let qtRow = qtRows.find(function(q) { return q[POST_SEL.PostUrlKey] == qtKey});
        if (qtRow) {
...
console.log(rows);
  ...
  infuseRegularImages: function(rows, graphMatchRhs) {
    ...
    const entPostTime = APPSCHEMA.SocialPostTime;
    const entRegImgCdnUrl = APPSCHEMA.SocialPostRegImgSourceUrl;
    const entRegImg64Url = APPSCHEMA.SocialPostRegImgBinary;
    
    const ptime = 'ptime';
    const regimgcdnurl = 'regimgcdnurl';
    ...
    const sql = `
    SELECT  ${ptime}.${entPostTime.SubjectCol} AS ${POST_SEL.PostUrlKey},
            ${regimgcdnurl}.${entRegImgCdnUrl.ObjectCol} AS ${POST_SEL.RegImgCdnUrl},
            ${reg64url}.${entRegImg64Url.ObjectCol} AS ${POST_SEL.RegImg64Url}
    FROM ${entPostTime.Name} ${ptime}
    LEFT JOIN ${entRegImgCdnUrl.Name} ${regimgcdnurl} ON ${regimgcdnurl}.${entRegImgCdnUrl.SubjectCol} = ${ptime}.${entPostTime.SubjectCol}
      AND ${regimgcdnurl}.${graphMatchRhs}
    LEFT JOIN ${entRegImg64Url.Name} ${reg64url} ON ${reg64url}.${entRegImg64Url.SubjectCol} = ${ptime}.${entPostTime.SubjectCol}
      AND ${reg64url}.${graphMatchRhs}
    WHERE ${ptime} IN ( ${delimParms} );
    `;
