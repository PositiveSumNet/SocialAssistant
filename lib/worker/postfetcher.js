...
var POSTFETCHER = {
  ...
  SYNC: {
    ...
    getTopicRatings: function(step) {
      ...
      const sql = `
      SELECT ${ptr}.${entTopicRating.SubjectCol} AS ${SYNC_COL.RATED_POST.PostId},
      ...
      rows = rows.map(function(r) {
        let row = {};
        row[SYNC_COL.RATED_POST.PostId] = r[SYNC_COL.RATED_POST.PostId];
    ...
    // post
      ...
      // same pattern as SYNC.getNetworkConns
      let markerFilter = '';
      if (STR.hasLen(step.marker)) {
        let parm = {key: '$marker', value: step.marker};
        bind.push(parm);
        // we ought to have a thread urlkey, but in any event fall back to post urlkey
        markerFilter = `  AND COALESCE(${pthread}.${entThreadUrlKey.ObjectCol}, ${ptime}.${entPostTime.SubjectCol}) > ${parm.key}`;
      }

      // the not #quoted check is b/c we already infuse quotes, so that takes care of them
      const sql = `;with cte AS (
        SELECT DISTINCT ${ptime}.${entPostTime.SubjectCol} AS ${POST_SEL.PostUrlKey}
        FROM ${entPostTime.Name} ${ptime}
      ...
      FROM ${entPostTime.Name} ${ptime}
      JOIN cte c ON c.${POST_SEL.PostUrlKey} = ${ptime}.${entPostTime.SubjectCol}
