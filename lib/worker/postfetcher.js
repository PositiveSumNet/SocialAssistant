...
var POSTFETCHER = {
  ...
  postSearchWorker: function(request, specificUrlKeys, urlKeysAs) {
    ...
    if (specificUrlKeys && specificUrlKeys.length === 0) {
      ...
      if (STR.hasLen(request.topic)) {
        ...
        topicParm = {key: `$topic`, value: `${request.topic}${SUBTOPIC_RATING_DELIM}%`};
        bind.push(topicParm);
      }
      topicCondition = POSTFETCHER.writeTopicCondition(topicParm, request.guessTopics);
      cteSql = (request.guessTopics == true) ? POSTFETCHER.writeTopicGuessCte(topicParm) : '';
    ...
    POSTFETCHER.infuseTopicRatingTags(rows, graphMatchRhs, topicParm, request.guessTopics);
    return rows;
  },

  writeTopicGuessCte: function(topicParm) {
    ...
    if (topicParm) {
      ...
      topicCondition = `WHERE ${tsub}.${entTopicSubtopic.SubjectCol} || '${TOPICS.RENDER_DELIM}' || ${tsub}.${entTopicSubtopic.ObjectCol} = ${topicParm.key}`;
    }
  ...
  writeTopicCondition: function(topicParm, guessTopics) {
    const entPostTime = APPSCHEMA.SocialPostTime;
  ...
  infuseTopicRatingTags: function(rows, graphMatchRhs, topicParm, guessTopics) {
    ...
    if (guessTopics == true) {
      const cteSql = POSTFETCHER.writeTopicGuessCte(topicParm);
      if (topicParm) {
        bind.push(topicParm);
      }
