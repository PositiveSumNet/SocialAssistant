...
var POSTFETCHER = {
  ...
  writeTopicGuessCte: function(equalsTopicParm) {
    ...
    if (equalsTopicParm) {
      ...
      topicCondition = `WHERE ${tsub}.${entTopicSubtopic.SubjectCol} || '${TOPICS.TOPIC_SUBTOPIC_COLON}' || ${tsub}.${entTopicSubtopic.ObjectCol} = ${equalsTopicParm.key}`;
    }
  ...
  getInUseSubtopics: function(site) {
  ...
  SYNC: {
    ...
    getTopicRatings: function(step) {
      ...
      const subtopics = POSTFETCHER.getInUseSubtopics(site);
      ...
      let subtopic = '';
      if (!STR.hasLen(step.marker)) {
        subtopic = subtopics[0];
      }
      else {
        subtopic = ES6.getNext(subtopics, step.marker);
      }
    ...
    getPosts: function(step) {
      ...
      const sql = `
      SELECT  COALESCE(${pthread}.${entThreadUrlKey.ObjectCol}, ${ptime}.${entPostTime.SubjectCol}) AS ${SYNC_COL.POSTS.MarkerUrlKey},
    ...
    getPostImgs: function(step) {
      ...
      // const cteBind = [];
      // const cteSql = POSTFETCHER.SYNC.writePostsCteAndPushParm(step, bind, graphMatchRhs);
      // const cteBound = DBORM.QUERYING.bindConsol(cteBind);
      // // Fetching directly (not in a single go) because we want to be able to INNER JOIN on each image table
      // // and we want to be able to return back a "marker" (even if empty images)
      // // and not lose our place when we come upon a post that lacks images entirely
      // // (i.e. better off allowing for empties and keeping to the same rules as for posts)
      // const targetUrlKeySql = `${cteSql}
      // SELECT *
      // FROM cte;
      // `;
      // const targetRows = DBORM.QUERYING.fetch(targetUrlKeySql, cteBound);

      // if (targetRows.length === 0) { return []; }
      // const targetRow = targetRows[0];
      // const markerUrlKey = targetRow[SYNC_COL.POST_IMGS.ThreadUrlKey] || targetRow[SYNC_COL.POST_IMGS.PostUrlKey];
      ...
      //   let sql = `${cteSql}
      //   SELECT  '${markerUrlKey}' AS ${SYNC_COL.POST_IMGS.MarkerUrlKey},
    ...
    getNextPostUrlKeys: function(step) {
      ...
      if (STR.hasLen(step.marker)) {
        ...
        markerFilter = `  AND COALESCE(${pthread}.${entThreadUrlKey.ObjectCol}, ${ptime}.${entPostTime.SubjectCol}) > ${parm.key}`;
      }
      ...
      const markerSql = `
      SELECT DISTINCT COALESCE(${pthread}.${entThreadUrlKey.ObjectCol}, ${ptime}.${entPostTime.SubjectCol}) AS ${SYNC_COL.POSTS.MarkerUrlKey}
      ${bodySql}
      ${markerFilter}
      ORDER BY COALESCE(${pthread}.${entThreadUrlKey.ObjectCol}, ${ptime}.${entPostTime.SubjectCol}) ASC
      LIMIT 1;
      `;
