...
var POSTFETCHER = {
    ...
    if (specificUrlKeys && specificUrlKeys.length === 0) {
      ...
      if (STR.hasLen(request.topic)) {
        ...
        if (request.guessTopics == true) {
          guessedTopicParm = {key: `$guessedTopic`, value: request.topic};
  ...
  writeTopicGuessCte: function(guessedTopicParm) {
    ...
    if (guessedTopicParm) {
      ...
      topicCondition = `WHERE ${tsub}.${entTopicSubtopic.SubjectCol} || '${TOPICS.TOPIC_SUBTOPIC_COLON}' || ${tsub}.${entTopicSubtopic.ObjectCol} = ${guessedTopicParm.key}`;
    }
