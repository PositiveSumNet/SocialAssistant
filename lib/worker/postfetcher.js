  ...
  postSearchWorker: function(request, specificUrlKeys) {
    ...
    if (specificUrlKeys && specificUrlKeys.length === 0) {
      ...
      if (request.guessTopics == true) {
        // a common table expression (cte) is used to come up with the list of tag keywords *if* guessing topic
        // note that we can guess topics across *all* even for the case where no topic is provided (so the bind is conditional)
        if (STR.hasLen(request.topic)) {
